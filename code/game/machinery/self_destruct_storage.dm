...
/obj/machinery/nuclear_cylinder_storage/physical_attack_hand(mob/user)
	if(is_operable(src) && locked && check_access(user))
		locked = FALSE
...
/obj/machinery/nuclear_cylinder_storage/attackby(obj/item/O, mob/user)
	if(!open && is_operable(src) && istype(O, /obj/item/card/id))
	...
	if(open && istype(O, /obj/item/nuclear_cylinder) && (length(cylinders) < max_cylinders))
		...
		if(do_after(user, 8 SECONDS, src) && open && (length(cylinders) < max_cylinders) && user.unEquip(O, src))
			user.visible_message(
...
/obj/machinery/nuclear_cylinder_storage/handle_mouse_drop(atom/over, mob/user)
	if(over == user && open && length(cylinders))
		user.visible_message(
			"\The [user] begins to extract \the [cylinders[1]].", 
			"You begin to extract \the [cylinders[1]]."
		)
		if(do_after(user, 7 SECONDS, src) && open && length(cylinders))
			user.visible_message(
				"\The [user] picks up \the [cylinders[1]].", 
				"You pick up \the [cylinders[1]]."
			)
			user.put_in_hands(cylinders[length(cylinders)])
