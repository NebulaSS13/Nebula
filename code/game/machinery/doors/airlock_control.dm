...
/obj/machinery/airlock_sensor/get_examine_strings(mob/user, distance, infix, suffix)
	. = ..()
	if(distance < 2)
		. += SPAN_INFO("The pressure indicator reads '[pressure? round(pressure, 0.1) : 0] kPa'.")
	if(master_cycling)
		. += SPAN_WARNING("It's warning that the master airlock is cycling!")
	if(alert)
		. += SPAN_WARNING("The low-pressure warning light is blinking!")
...
/obj/machinery/button/access/get_examine_strings(mob/user, distance, infix, suffix)
	. = ..()
	if(distance <= 1)
		. += SPAN_INFO("The button reads '[command]'.")
	if(master_cycling)
		. += SPAN_WARNING("It's warning that the master airlock is cycling!")
