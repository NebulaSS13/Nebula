...
/obj/machinery/porta_turret/attackby(obj/item/I, mob/user)
	if(stat & BROKEN)
		...
		if(enabled || raised)
			to_chat(user, "<span class='warning'>You cannot unsecure an active turret!")
			return
		if(wrenching)
			to_chat(user, "<span class='warning'>Someone is already [anchored ? "un" : ""]securing the turret!")
			return
		if(!anchored && isspaceturf(get_turf(src)))
			to_chat(user, "<span class='warning'>Cannot secure turrets in space!")
			return

		user.visible_message( \
				"<span class='warning'>[user] begins [anchored ? "un" : ""]securing the turret.", \
...
/obj/machinery/porta_turret/emag_act(var/remaining_charges, var/mob/user)
	if(!emagged)
		...
		to_chat(user, "<span class='warning'>You short out [src]'s threat assessment circuits.")
	...
	switch(build_step)
		if(0)	//first step
			if(isWrench(I) && !anchored)
				if(I.do_tool_interaction(TOOL_WRENCH, user, src, 0, "anchoring", "anchor"))
					anchored = 1
					build_step = 1
					return

			else if(isCrowbar(I) && !anchored)
				if(I.do_tool_interaction(TOOL_CROWBAR, user, src, 0, "dismantle", "dismantle"))
		...
		if(3)
			if(istype(I, /obj/item/gun/energy)) //the gun installation part
				...
				if(I.do_tool_interaction(TOOL_WRENCH, user, src, 0, "remove the bolts", "removing the bolts"))
					build_step = 2
					return
		...
		if(5)
			if(isScrewdriver(I))
				if(I.do_tool_interaction(TOOL_SCREWDRIVER, user, src, 0, "closing the internal hatch", "closing the internal hatch"))
					build_step = 6
		...
		if(6)
			if(istype(I, /obj/item/stack/material) && I.get_material_type() == /decl/material/solid/metal/steel)
				...
				if(M.use(2))
					...
					to_chat(user, "<span class='warning'>You need two sheets of metal to continue construction.")
				return
		...
		if(7)
			if(isWelder(I))
				if(I.do_tool_interaction(TOOL_WELDER, user, src, 3 SECONDS, "weld the armor down on", "welding the armor down on", fuel_expenditure = 5))
					build_step = 8
