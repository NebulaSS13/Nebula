		...
		switch(removal_stage)
			if(0)
				if(isWelder(O))
					...
					if(do_after(user,40 * O.tool_speed_mult, src))
			...
			if(1)
				if(isCrowbar(O))
					...
					if(do_after(user, 15 * O.tool_speed_mult, src))
						if(!src || !user) return
			...
			if(3)
				if(isWrench(O))
					...
					if(do_after(user, 50 * O.tool_speed_mult, src))
						if(!src || !user) return
			...
			if(4)
				if(isCrowbar(O))
					...
					if(do_after(user, 80 * O.tool_speed_mult, src))
						if(!src || !user) return
