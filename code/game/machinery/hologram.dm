...
/obj/machinery/hologram/holopad/Initialize()
	...
	desc = "It's a floor-mounted device for projecting holographic images. Its ID is '[A.display_name]'"
			...
			if(last_request + 200 < world.time) //don't spam the AI with requests you jerk!
				...
				for(var/mob/living/silicon/ai/AI in global.living_mob_list_)
					...
					if (holopadType != HOLOPAD_LONG_RANGE && !AreConnectedZLevels(AI.z, src.z))
						continue
					to_chat(AI, "<span class='info'>Your presence is requested at <a href='?src=\ref[AI];jumptoholopad=\ref[src]'>\the [area.display_name]</a>.</span>")
			else
			...
			if(last_request + 200 < world.time) //don't spam other people with requests either, you jerk!
				...
				for(var/obj/machinery/hologram/holopad/H in SSmachines.machinery)
					if (H.operable())
						...
						if(H.z in zlevels)
							holopadlist["[A.display_name]"] = H	//Define a list and fill it with the area of every holopad in the world
						if (H.holopadType == HOLOPAD_LONG_RANGE && (H.z in zlevels_long))
							holopadlist["[A.display_name]"] = H
...
/obj/machinery/hologram/holopad/proc/make_call(var/obj/machinery/hologram/holopad/targetpad, var/mob/living/carbon/user)
	...
	targetpad.audible_message("<b>\The [src]</b> announces, \"Incoming communications request from [our_area.display_name].\"")
	to_chat(user, "<span class='notice'>Trying to establish a connection to the holopad in [target_area.display_name]... Please await confirmation from recipient.</span>")
...
/obj/machinery/hologram/holopad/proc/take_call(mob/living/carbon/user)
	...
	var/area/our_area = get_area(src)
	log_admin("[key_name(caller_id)] just established a holopad connection from [source_area.display_name] to [our_area.display_name]")
