...
/obj/machinery/vitals_monitor/Process()
	if(QDELETED(victim) || (victim && !Adjacent(victim)))
		update_victim()
...
/obj/machinery/vitals_monitor/proc/handle_pulse()
	...
	if(istype(heart) && !BP_IS_PROSTHETIC(heart))
		switch(victim.pulse())
			if(PULSE_NONE)
...
/obj/machinery/vitals_monitor/attack_hand(mob/user)
	if(CanPhysicallyInteract(user))
