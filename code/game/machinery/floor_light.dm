...
/obj/machinery/floor_light/attackby(var/obj/item/W, var/mob/user)
	if(isScrewdriver(W))
		...
		if(W.do_tool_interaction(TOOL_WELDER, user, src, 2 SECONDS, "begin repairing", "repairing", fuel_expenditure = 1))
			visible_message(SPAN_NOTICE("\The [user] has repaired \the [src]."))
			set_broken(FALSE)
			damaged = null
	else if(isWrench(W))
		if(W.do_tool_interaction(TOOL_WRENCH, user, src, 0, "dismantling", "dismantling"))
