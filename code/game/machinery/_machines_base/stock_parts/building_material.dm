...
/obj/item/stock_parts/building_material/proc/add_material(var/obj/item/new_material)
	if(istype(new_material, /obj/item/stack))
		...
		for(var/obj/item/stack/old_stack in building_materials)
			if(stack.transfer_to(old_stack) && QDELETED(stack))
				return
	LAZYADD(building_materials, new_material)
	...
	if(ispath(material_type, /obj/item/stack))
		for(var/obj/item/stack/stack in building_materials)
			if(stack.stacktype == material_type)
				...
				return new_stack
	for(var/obj/item/item in building_materials)
		if(istype(item, material_type))
			building_materials -= item
			item.dropInto(loc)
			return item

/obj/item/stock_parts/building_material/on_uninstall(var/obj/machinery/machine)
	for(var/obj/item/I in building_materials)
		I.dropInto(loc)
