...
/obj/machinery/power/supply_beacon/attackby(var/obj/item/W, var/mob/user)
	if(!use_power && isWrench(W))
		if(!anchored && !connect_to_network())
			to_chat(user, SPAN_DANGER("This device must be placed over an exposed cable."))
			return
		anchored = !anchored
...
/obj/machinery/power/supply_beacon/physical_attack_hand(var/mob/user)
	if(expended)
		...
		to_chat(user, SPAN_DANGER("\The [src] has used up its charge."))
		return TRUE
	...
	if(anchored)
		...
		to_chat(user, SPAN_DANGER("You need to secure the beacon with a wrench first!"))
		return TRUE
