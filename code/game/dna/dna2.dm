...
/datum/dna/proc/ResetUIFrom(var/mob/living/carbon/human/character)
	...
	var/hair = GET_DECL(character.h_style)

	// Facial Hair
	if(!character.f_style)
		character.f_style = /decl/sprite_accessory/facial_hair/shaved
	var/beard = GET_DECL(character.f_style)
	...
	SetUIValueRange(DNA_UI_HAIR_STYLE,  hair,  length(subtypesof(/decl/sprite_accessory/hair)), 1)
	SetUIValueRange(DNA_UI_BEARD_STYLE, beard, length(subtypesof(/decl/sprite_accessory/facial_hair)), 1)
