...
/decl/game_mode/Topic(href, href_list[])
	...
	if(href_list["toggle"])
		...
		if(href_list["debug_antag"] == "self")
			...
			return
		var/decl/special_role/antag = decls_repository.get_decl_by_id(href_list["debug_antag"])
		if(antag)
			usr.client.debug_variables(antag)
			message_admins("Admin [key_name_admin(usr)] is debugging the [antag.name] template.")
	else if(href_list["remove_antag_type"])
		var/decl/special_role/antag = decls_repository.get_decl_by_id(href_list["remove_antag_type"])
		if(!antag)
			return
