...
/obj/effect/rune/armor/cast(var/mob/living/user)
	...
	var/obj/O = user.get_equipped_item(BP_HEAD) // This will most likely kill you if you are wearing a spacesuit, and it's 100% intended
	if(O && !istype(O, /obj/item/clothing/head/culthood) && user.unEquip(O))
		user.equip_to_slot_or_del(new /obj/item/clothing/head/culthood/alt(user), BP_HEAD)
	O = user.get_equipped_item(BP_BODY)
	if(O && !istype(O, /obj/item/clothing/suit/cultrobes) && user.unEquip(O))
		user.equip_to_slot_or_del(new /obj/item/clothing/suit/cultrobes/alt(user), BP_BODY)
	...
	O = user.get_equipped_item(BP_SHOULDERS)
	if(istype(O, /obj/item/storage) && !istype(O, /obj/item/storage/backpack/cultpack) && user.unEquip(O)) // We don't want to make the vox drop their nitrogen tank, though
		...
		user.equip_to_slot_or_del(C, BP_SHOULDERS)
		if(C)
...
/obj/effect/rune/weapon/cast(var/mob/living/user)
	if(!istype(user.get_equipped_item(BP_HEAD), /obj/item/clothing/head/culthood) || !istype(user.get_equipped_item(BP_BODY), /obj/item/clothing/suit/cultrobes) || !istype(user.get_equipped_item(slot_shoes_str), /obj/item/clothing/shoes/cult))
