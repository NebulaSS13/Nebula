...
/obj/item/get_material_composition()
	return material_composition

/obj/item/set_material_composition(var/list/new_materials)
...
/obj/item/Initialize(var/ml, var/material_key)
	. = ..()
	create_material_composition(ispath(material_key, /decl/material) && material_key)
	if(islist(armor))
...
/obj/item/examine(mob/user, distance)
	...
	if(hasHUD(user, HUD_SCIENCE)) //Mob has a research scanner active.
		...
		if(istype(matter) && length(matter))
