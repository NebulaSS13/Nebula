/obj/item
	...
	var/list/tool_types //List of tool qualities. i.e list(TOOL_FLAG_WRENCH = 1.5), with the number being how much faster or slower the tool is. Do not use negative numbers.
	var/current_tool_type //What tool are we being used for currently?

	var/tool_speed_mult = 1
...
/obj/item/proc/change_tool_type(var/user)
...
/obj/item/proc/get_tool_speed(var/user)
	return tool_types[current_tool_type]

/obj/item/proc/do_tool_interaction(var/req_tool_type, var/req_tool_quality, var/base_delay, var/user, var/interacting_atom, var/start_text, var/finish_text, var/fuel)
	if(current_tool_type != req_tool_type)
		return

	if(tool_types[req_tool_type] < req_tool_quality)
		to_chat(user, SPAN_WARNING("This tool isn't good enough for this action."))
		return

	playsound(get_turf(interacting_atom), pick(tool_sound), 75, 1)
	to_chat(user, SPAN_NOTICE("[start_text]"))

	if(base_delay <= 0) //equal to or less than zero = return TRUE instantly to avoid div by zero.
		to_chat(user, SPAN_NOTICE("[finish_text]"))
		return TRUE

	if(do_after(user, base_delay / get_tool_speed(user), interacting_atom))
		to_chat(user, SPAN_NOTICE("[finish_text]"))
		return TRUE
	else
		return FALSE
