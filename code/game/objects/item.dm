...
/obj/item/proc/do_tool_interaction(var/req_tool_type, var/req_tool_quality, var/base_delay, var/user, var/interacting_atom, var/start_text, var/finish_text, var/fuel)
	var/datum/extension/tool = get_extension(src, /datum/extension/tool)

	if(isnull(tool))
		return

	if(tool?.current_tool_type != req_tool_type)
		return

	if(tool?.tool_types[req_tool_type] < req_tool_quality)
		to_chat(user, SPAN_WARNING("This tool isn't good enough for this action."))
		return

	playsound(get_turf(interacting_atom), pick(tool?.tool_sounds), 75, 1)
	...
	if(do_after(user, base_delay / tool?.get_tool_speed(user), interacting_atom))
		to_chat(user, SPAN_NOTICE("[finish_text]"))
		return TRUE
	else
		return FALSE
