...
/obj/structure/hygiene/Process()
	...
	if(flood_amt)
		...
		if(istype(T))
			...
			if(adding)
				T.add_fluid(adding, /decl/material/water)
	...
	for(var/thing in loc)
		...
		if(istype(AM) && AM.simulated)
			...
			if(istype(L))
				process_heat(L)
	wash_floor()
	reagents.add_reagent(/decl/material/water, REAGENTS_FREE_SPACE(reagents))
...
/obj/structure/hygiene/sink/attackby(obj/item/O, var/mob/living/user)
	...
	if (istype(RG) && ATOM_IS_OPEN_CONTAINER(RG) && RG.reagents)
		RG.reagents.add_reagent(/decl/material/water, min(RG.volume - RG.reagents.total_volume, RG.amount_per_transfer_from_this))
		...
		O.reagents.add_reagent(/decl/material/water, 5)
