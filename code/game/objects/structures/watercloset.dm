	...
	for(var/thing in loc)
		...
		if(istype(AM) && AM.simulated)
			...
			if(istype(L))
				process_heat(L)
	wash_floor()
	reagents.add_reagent(MAT_WATER, REAGENTS_FREE_SPACE(reagents))
...
/obj/structure/hygiene/sink/attackby(obj/item/O, var/mob/living/user)
	...
	if (istype(RG) && ATOM_IS_OPEN_CONTAINER(RG) && RG.reagents)
		RG.reagents.add_reagent(MAT_WATER, min(RG.volume - RG.reagents.total_volume, RG.amount_per_transfer_from_this))
		...
		O.reagents.add_reagent(MAT_WATER, 5)
