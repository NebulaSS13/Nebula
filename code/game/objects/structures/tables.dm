...
/obj/structure/table/on_update_icon()
	...
	if(!is_flipped)
		...
		for(var/direction in list(turn(dir,90), turn(dir,-90)) )
			...
			if(blend_with(T) && T.is_flipped == is_flipped && T.dir == dir)
				flip_type++
				tabledirs |= direction
		if(flip_type == 1)
			...
			if(tabledirs & turn(dir,-90))
				icon_state += "+"
		var/image/I = image(icon, "flip[flip_type]")
		I.color = material.color
		I.alpha = 255 * material.opacity
		add_overlay(I)
		if(reinf_material)
			I = image(icon, "[reinf_material.table_icon_base]_flip[flip_type]")
			I.color = reinf_material.color
			I.alpha = 255 * reinf_material.opacity
			add_overlay(I)
		if(additional_reinf_material)
			I = image(icon, "[reinf_material.table_icon_reinforced]_flip[flip_type]")
			I.color = additional_reinf_material.color
			I.alpha = 255 * additional_reinf_material.opacity
			add_overlay(I)
		if(felted)
			add_overlay("carpet_flip[flip_type]")
...
/obj/structure/table/proc/recursive_flip_check(var/direction, var/list/checked)
	...
	// Is the table directly in the directio of flipping part of us?
	...
	for(var/angle in list(-90, 90))
		...
		if(T in checked)
			continue
		if(!blend_with(T) || !T.recursive_flip_check(direction, checked))
			return FALSE
