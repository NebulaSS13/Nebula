		...
		switch(href_list["set"])
			...
			if("Factory Reset")
				if(alert("This will factory reset the card, including access and owner. Continue?", "Factory Reset", "No", "Yes") == "Yes" && CanUseTopic(user, state))
					...
					. = 1
			if("Branch")
				var/new_branch = sanitize(input(user,"What branch of service would you like to put on this card?","Agent Card Branch") as null|anything in global.using_map.mil_branches.spawn_branches())
				if(!isnull(new_branch) && CanUseTopic(user, state))
					src.military_branch =  global.using_map.mil_branches.spawn_branches()[new_branch]
					to_chat(user, "<span class='notice'>Branch changed to '[military_branch.name]'.</span>")
					. = 1
			if("Rank")
				var/new_rank = sanitize(input(user,"What rank would you like to put on this card?","Agent Card Rank") as null|anything in global.using_map.mil_branches.spawn_ranks(military_branch.name))
				if(!isnull(new_rank) && CanUseTopic(user, state))
					src.military_rank = global.using_map.mil_branches.spawn_ranks(military_branch.name)[new_rank]
