...
/obj/item/rpd/interact(mob/user)
	...
	popup.set_content(get_console_data(user.skill_check(SKILL_CONSTRUCTION,SKILL_EXPERT) ? GLOB.rpd_pipe_selection_skilled : GLOB.rpd_pipe_selection, TRUE))
	popup.open()
...
/obj/item/rpd/afterattack(atom/A, mob/user, proximity)
	...
	if(istype(A, /obj/item/pipe))
		...
		if(user.skill_fail_prob(SKILL_CONSTRUCTION, 80, SKILL_ADEPT))
