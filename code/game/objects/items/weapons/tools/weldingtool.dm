...
/obj/item/weldingtool/proc/get_fuel()
	return tank ? REAGENT_VOLUME(tank.reagents, /decl/material/fuel) : 0
...
/obj/item/weldingtool/proc/burn_fuel(var/amount)
	...
	if(in_mob)
		amount = max(amount, 2)
		tank.reagents.trans_type_to(in_mob, /decl/material/fuel, amount)
		in_mob.IgniteMob()

	else
		tank.reagents.remove_reagent(/decl/material/fuel, amount)
...
/obj/item/welder_tank/experimental/Process()
	var/cur_fuel = REAGENT_VOLUME(reagents, /decl/material/fuel)
	if(cur_fuel < max_fuel)
		var/gen_amount = ((world.time-last_gen)/25)
		reagents.add_reagent(/decl/material/fuel, gen_amount)
		last_gen = world.time