...
/obj/item/flame/lighter/attack_self(mob/living/user)
	if(!lit)
		if(reagents.has_reagent(MAT_FUEL))
			light(user)
		else
...
/obj/item/flame/lighter/Process()
	if(!submerged() && reagents.has_reagent(MAT_FUEL))
		if(ismob(loc) && prob(10) && REAGENT_VOLUME(reagents, MAT_FUEL) < 1)
			to_chat(loc, "<span class='warning'>\The [src]'s flame flickers.</span>")
			set_light(0)
			addtimer(CALLBACK(src, .atom/proc/set_light, 0.6, 0.5, 2), 4)
		reagents.remove_reagent(MAT_FUEL, 0.05)
	else
		extinguish()
		return
