...
/obj/item/canvas
	...
	icon = 'icons/obj/items/canvas.dmi'
	icon_state = "11x11"
...
/obj/item/canvas/attackby(obj/item/I, mob/user, params)
	if (istype(I, /obj/item/wrench))
		...
		to_chat(user,"<span class='notice'>You begin to [anchored ? "loosen" : "tighten"] \the [src]...</span>")
		if (do_after(user, 40, src))
			user.visible_message( \
				"\The [user] [anchored ? "loosens" : "tightens"] \the [src].", \
				SPAN_NOTICE("You have [anchored ? "loosened" : "tightened"] \the [src]. It is [anchored ? "now secured" : "moveable"]."), \
				"You hear a ratchet.")
			anchored = !anchored
		return
	...
	if(istype(I, /obj/item/soap) || istype(I, /obj/item/chems/glass/rag))
...
/obj/item/canvas/attack_self(mob/user)
	...
	if(blank)
		...
		icon = blank
		user.visible_message(
			SPAN_NOTICE("\The [user] cleans the canvas."), 
			SPAN_NOTICE("You clean the canvas."))
