/obj/item
	...
	var/minimum_size_to_twohand = MOB_SIZE_MEDIUM
	var/maximum_size_to_twohand = MOB_SIZE_LARGE
	var/last_twohanded_state    = FALSE
...
/obj/item/proc/update_twohanding()
	...
	var/next_wielded_state = is_held_twohanded()
	if(last_twohanded_state == next_wielded_state)
		return

	if(next_wielded_state)
		if(wieldsound)
	...
	update_held_icon()
	update_attack_force()
	last_twohanded_state = next_wielded_state
...
/obj/item/proc/is_held_twohanded(mob/living/wielder)
	if(!can_be_twohanded)
		return FALSE
	...
	if(!istype(wielder))
		return FALSE
	if(!(src in wielder.get_held_items()))
		return FALSE
