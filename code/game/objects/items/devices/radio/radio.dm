...
/obj/item/radio/Initialize()
	...
	set_extension(src, /datum/extension/network_device/radio, initial_network_id, initial_network_key, NETWORK_CONNECTION_WIRELESS)
...
/obj/item/radio/proc/transmit(var/mob/speaker, message, message_mode, var/verb = "says", var/decl/language/speaking = null)
	...
	var/obj/send_overmap_object = istype(T) && map_sectors["[T.z]"]
	for(var/mob/listener in send_message_to)
		if(send_overmap_object && send_overmap_object == send_message_to[listener])
			...
			if(sender_obj && (!istype(sender_obj) || sender_obj.ident_transmitter))
				send_name = "[send_name] ([sender_obj.name])"
		listener.hear_radio(send_message, verb, speaking, formatted_msg, "</span> <span class='message'>", "</span></span>", speaker, message_compression, send_name)
...
/obj/item/radio/attackby(obj/item/W, mob/user)
	...
	if(isScrewdriver(W) && length(encryption_keys))
