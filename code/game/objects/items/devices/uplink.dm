...
/* How to create an uplink in 3 easy steps!
 ...
 2. Code in the triggers. Use check_trigger for this, I recommend closing the item's menu with "close_browser(usr, "windowname") if it returns true.
...
/obj/item/uplink/proc/trigger(mob/user as mob)
	if(!active)
		toggle()
	interact(user)
...
// current item's menu.
/obj/item/uplink/proc/check_trigger(mob/user as mob, var/value, var/target)
	if(value == target)
		trigger(user)
		return 1
	return 0
...
// If it returns true, I recommend closing the item's normal menu with "close_browser(user, "name")"
