...
/datum/job/proc/get_branch_rank(var/decl/species/S)
	...
	var/datum/mil_branches/branches = global.using_map.mil_branches

	var/spawn_branches = branches.spawn_branches(S)
	for(var/branch_type in allowed_branches)
		var/datum/mil_branch/branch = branches.get_branch_by_type(branch_type)
...
/datum/job/proc/is_branch_allowed(var/branch_name)
	...
	var/datum/mil_branch/branch = global.using_map.mil_branches.get_branch(branch_name)
...
/datum/job/proc/is_rank_allowed(var/branch_name, var/rank_name)
	...
	var/datum/mil_rank/rank = global.using_map.mil_branches.get_rank(branch_name, rank_name)
...
/datum/job/proc/get_branches()
	...
	for(var/branch in allowed_branches)
		var/datum/mil_branch/branch_datum = global.using_map.mil_branches.get_branch_by_type(branch)
		. += branch_datum.name
	return english_list(.)
...
/datum/job/proc/get_ranks(branch)
	. = list()
	var/datum/mil_branch/branch_datum = global.using_map.mil_branches.get_branch(branch)
