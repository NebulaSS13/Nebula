		...
		if(!islist(species.default_cultural_info))
			...
			for(var/token in species.default_cultural_info)
				if(!(token in ALL_CULTURAL_TAGS))
					...
					if(!val)
						fails++
						log_bad("Default cultural value '[val]' for [species_name] tag '[token]' is null, must be a type.")
					else if(istext(val))
						fails++
						log_bad("Default cultural value '[val]' for [species_name] tag '[token]' is text, must be a type.")
					else
						var/decl/cultural_info/culture = ispath(val, /decl/cultural_info) && decls_repository.get_decl(val)
						if(!istype(culture))
							fails++
		...
		if(!islist(species.force_cultural_info))
			...
			for(var/token in species.force_cultural_info)
				if(!(token in ALL_CULTURAL_TAGS))
					...
					if(!val)
						fails++
						log_bad("Forced cultural value for [species_name] tag '[token]' is null, must be a type.")
					else if(istext(val))
						fails++
