...
/datum/unit_test/bodytype_organ_creation/proc/check_internal_organs(var/mob/living/carbon/human/H, var/decl/bodytype/bodytype)
	. = 1
...
/datum/unit_test/bodytype_organ_creation/proc/check_external_organs(var/mob/living/carbon/human/H, var/decl/bodytype/bodytype)
	. = 1
...
/datum/unit_test/bodytype_organ_creation/proc/check_organ_parents(var/mob/living/carbon/human/H, var/decl/bodytype/bodytype)
	. = 1
...
/datum/unit_test/bodytype_organ_lists_update/proc/check_internal_organ_present(var/mob/living/carbon/human/H, var/obj/item/organ/internal/I)
	if(!(I in H.get_internal_organs()))
		fail("[H.bodytype.name] internal organ [I] not in internal_organs.")
		return 0
	var/found = GET_INTERNAL_ORGAN(H, I.organ_tag)
	if(I != found)
		fail("[H.bodytype.name] internal organ [I] not in organ list. Organ tag was \"[I.organ_tag]\", found [found? found : "nothing"] instead.")
		return 0
	var/obj/item/organ/external/parent = GET_EXTERNAL_ORGAN(H, I.parent_organ)
	if(!istype(parent))
		fail("[H.bodytype.name] internal organ [I] could not find its parent in organs_by_name. Parent tag was \"[I.parent_organ]\".")
		return 0
	if(!(I in parent.internal_organs))
		fail("[H.bodytype.name] internal organ [I] was not in parent's internal_organs. Parent was [parent]. Parent tag was \"[I.parent_organ]\".")
		return 0
	return 1

/datum/unit_test/bodytype_organ_lists_update/proc/check_internal_organ_removed(var/mob/living/carbon/human/H, var/obj/item/organ/internal/I, var/obj/item/organ/external/old_parent)
	if(I in H.get_internal_organs())
		fail("[H.bodytype.name] internal organ [I] was not removed from internal_organs.")
		return 0
	var/found = GET_INTERNAL_ORGAN(H, I.organ_tag)
	if(found)
		fail("[H.bodytype.name] internal organ [I] was not removed from organ list. Organ tag was \"[I.organ_tag]\".")
		return 0
	if(I in old_parent.internal_organs)
		fail("[H.bodytype.name] internal organ [I] was not removed from parent's internal_organs. Parent was [old_parent].")
		return 0
	return 1

/datum/unit_test/bodytype_organ_lists_update/proc/check_external_organ_present(var/mob/living/carbon/human/H, var/obj/item/organ/external/E)
	if(!(E in H.get_external_organs()))
		fail("[H.bodytype.name] external organ [E] not in organs.")
		return 0
	var/found = GET_EXTERNAL_ORGAN(H, E.organ_tag)
	if(E != found)
		fail("[H.bodytype.name] external organ [E] not in organ list. Organ tag was \"[E.organ_tag]\", found [found? found : "nothing"] instead.")
		return 0
	if(E.parent_organ)
		var/obj/item/organ/external/parent = E.parent
		if(!istype(parent))
			fail("[H.bodytype.name] external organ [E] had no parent. Parent tag was \"[E.parent_organ]\".")
			return 0
		if(parent.organ_tag != E.parent_organ)
			fail("[H.bodytype.name] external organ [E] parent tag mismatch. Parent tag was \"[E.parent_organ]\", actual tag was \"[parent.organ_tag]\".")
			return 0
		if(!(E in parent.children))
			fail("[H.bodytype.name] external organ [E] was not in parent's children. Parent was [parent]. Parent tag was \"[E.parent_organ]\".")
			return 0
	return 1

/datum/unit_test/bodytype_organ_lists_update/proc/check_external_organ_removed(var/mob/living/carbon/human/H, var/obj/item/organ/external/E, var/obj/item/organ/external/old_parent = null)
	if(E in H.get_external_organs())
		fail("[H.bodytype.name] external organ [E] was not removed from organs.")
		return 0
	var/found = GET_EXTERNAL_ORGAN(H, E.organ_tag)
	if(found)
		fail("[H.bodytype.name] external organ [E] was not removed from organs_by_name. Organ tag was \"[E.organ_tag]\".")
		return 0
	if(old_parent)
		if(!(E in old_parent.children))
			fail("[H.bodytype.name] external organ [E] was not removed from parent's children. Parent was [old_parent].")
			return 0
	return 1

/datum/unit_test/bodytype_organ_lists_update/proc/test_internal_organ(var/mob/living/carbon/human/H, var/obj/item/organ/internal/I)
	if(!check_internal_organ_present(H, I))
		fail("[H.bodytype.name] internal organ [I] failed initial presence check.")
		return 0
	...
	if(!check_internal_organ_removed(H, I, parent))
		fail("[H.bodytype.name] internal organ [I] was not removed correctly.")
		return 0

	H.add_organ(I, parent)
	if(!check_internal_organ_present(H, I))
		fail("[H.bodytype.name] internal organ [I] was not replaced correctly.")
		return 0

	return 1

/datum/unit_test/bodytype_organ_lists_update/proc/test_external_organ(var/mob/living/carbon/human/H, var/obj/item/organ/external/E)
	if(!check_external_organ_present(H, E))
		fail("[H.bodytype.name] external organ [E] failed initial presence check.")
		return 0
	...
	if(!check_external_organ_removed(H, E, parent))
		fail("[H.bodytype.name] external organ [E] was not removed correctly.")
		return 0

	H.add_organ(E)
	if(!check_external_organ_removed(H, E))
		fail("[H.bodytype.name] external organ [E] was not replaced correctly.")
		return 0
