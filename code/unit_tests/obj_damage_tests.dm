...
/datum/item_unit_test/constant/items_shall_stay_invincible/run_test(var/obj/item/I)
	if(initial(I.health) != ITEM_HEALTH_NO_DAMAGE && initial(I.obj_max_health) != ITEM_HEALTH_NO_DAMAGE)
		return TRUE //Ignore things that aren't invincible
	if(I.health != ITEM_HEALTH_NO_DAMAGE || I.get_max_health() != ITEM_HEALTH_NO_DAMAGE)
...
/datum/item_unit_test/constant/items_shall_define_their_max_health/run_test(var/obj/item/I)
	...
	if(I.health == ITEM_HEALTH_NO_DAMAGE || current_max_health == ITEM_HEALTH_NO_DAMAGE)
		return TRUE //We don't care about invincible things
	if(I.health > 0 && current_max_health != I.health)
		IT.report_failure(src, I.type, "Defines health = [I.health], but its max health is [current_max_health || "null"].")
		return FALSE
	return TRUE
...
/datum/item_unit_test/constant/items_shall_get_a_valid_health_from_materials/run_test(var/obj/item/I)
	...
	if(!isnull(initial(I.health)) || !isnull(initial(I.obj_max_health)) || !I.material)
		return TRUE

	if((I.get_max_health() < 1.0) || (I.health < 1.0))
...
/datum/item_unit_test/volatile/items_shall_take_damage/run_test(var/obj/item/I)
	...
	if(!isnull(old_health))
		if(old_health && old_health == I.health && old_health != ITEM_HEALTH_NO_DAMAGE && I.get_max_health() != ITEM_HEALTH_NO_DAMAGE)
		...
		if(old_health != I.health && (old_health == ITEM_HEALTH_NO_DAMAGE || I.get_max_health() == ITEM_HEALTH_NO_DAMAGE))
	...
	var/damage_taken_actual = (old_health == ITEM_HEALTH_NO_DAMAGE || I.get_max_health() == ITEM_HEALTH_NO_DAMAGE || isnull(old_health))? 0 : old_health - I.health
