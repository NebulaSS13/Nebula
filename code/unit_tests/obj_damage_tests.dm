...
/datum/item_unit_test/constant/items_shall_stay_invincible/run_test(var/obj/item/I)
	if(initial(I.health) != OBJ_HEALTH_NO_DAMAGE && initial(I.max_health) != OBJ_HEALTH_NO_DAMAGE)
		return TRUE //Ignore things that aren't invincible
	if(I.health != OBJ_HEALTH_NO_DAMAGE || I.max_health != OBJ_HEALTH_NO_DAMAGE)
...
/datum/item_unit_test/volatile/items_shall_take_damage/run_test(var/obj/item/I)
	...
	if(!isnull(old_health))
		if(old_health && old_health == I.health && old_health != OBJ_HEALTH_NO_DAMAGE && I.max_health != OBJ_HEALTH_NO_DAMAGE)
		...
		if(old_health != I.health && (old_health == OBJ_HEALTH_NO_DAMAGE || I.max_health == OBJ_HEALTH_NO_DAMAGE))
	...
	var/damage_taken_actual = (old_health == OBJ_HEALTH_NO_DAMAGE || I.max_health == OBJ_HEALTH_NO_DAMAGE || isnull(old_health))? 0 : old_health - I.health
