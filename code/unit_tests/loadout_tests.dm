...
/datum/unit_test/loadout_test_shall_have_name_cost_path/start_test()
	...
	if(length(failed))
		fail("[length(failed)] /datum/gear definition\s had invalid display names, costs, or path definitions:\n[jointext(failed, "\n")]")
	else
		pass("All /datum/gear definitions had valid display names, costs and path definitions.")
	return  1
...
/datum/unit_test/loadout_test_shall_have_valid_icon_states/start_test()
	...
	if(length(failed))
		fail("[length(failed)] /datum/gear definition\s had paths with invalid icon states:\n[jointext(failed, "\n")]")
	else
