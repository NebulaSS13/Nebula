		...
		for (i in 1 to L)
			...
			if (timer.timeToRun < head_offset + TICKS2DS(practical_offset-1))
				...
				if (timer.timeToRun < head_offset)
					PRINT_STACK_TRACE("[i] Invalid timer state: Timer in long run queue with a time to run less then head_offset. [get_timer_debug_string(timer)] world.time: [world.time], head_offset: [head_offset], practical_offset: [practical_offset]")
				else
					PRINT_STACK_TRACE("[i] Invalid timer state: Timer in long run queue that would require a backtrack to transfer to short run queue. [get_timer_debug_string(timer)] world.time: [world.time], head_offset: [head_offset], practical_offset: [practical_offset]")
...
/proc/addtimer(datum/callback/callback, wait = 0, flags = 0)
	...
	if (callback.object != GLOBAL_PROC && QDELETED(callback.object) && !QDESTROYING(callback.object))
		PRINT_STACK_TRACE("addtimer called with a callback assigned to a qdeleted object. In the future such timers will not be supported and may refuse to run or run with a 0 wait")
