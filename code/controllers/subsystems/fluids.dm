...
/datum/controller/subsystem/fluids/fire(resumed = 0)
	...
	while(i < processing_fluids.len)
		...
		if(isspaceturf(current_fluid_holder) || (istype(current_fluid_holder, /turf/floor) && current_fluid_holder.is_outside() && (current_fluid_holder.reagents?.total_volume + current_fluid_holder.get_physical_height()) > 0))
