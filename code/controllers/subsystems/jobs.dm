	...
	for(var/mob/new_player/player in unassigned_roundstart)
		if(player.client.prefs.alternate_option == BE_ASSISTANT)
			...
			if((global.using_map.flags & MAP_HAS_BRANCH) && player.client.prefs.branches[initial(ass.title)])
				var/datum/mil_branch/branch = global.using_map.mil_branches.get_branch(player.client.prefs.branches[initial(ass.title)])
	...
	if(job)
		if(H.client)
			if(global.using_map.flags & MAP_HAS_BRANCH)
				H.char_branch = global.using_map.mil_branches.get_branch(H.client.prefs.branches[job_title])
			if(global.using_map.flags & MAP_HAS_RANK)
				H.char_rank = global.using_map.mil_branches.get_rank(H.client.prefs.branches[job_title], H.client.prefs.ranks[job_title])
