...
SUBSYSTEM_DEF(runechat)
	...
	var/list/datum/runechat/second_queue = list()
...
/datum/controller/subsystem/runechat/fire(resumed = FALSE)
	...
	var/static/datum/runechat/cm
	if (!resumed)
		cm = null

	// Iterate through each bucket starting from the practical offset
	while (practical_offset <= BUCKET_LEN && head_offset + ((practical_offset - 1) * world.tick_lag) <= world.time)
		var/datum/runechat/bucket_head = bucket_list[practical_offset]
		...
		while (cm)
			...
			var/datum/runechat/next = cm.next
			if (!cm.eol_complete)
				cm.end_of_life()
 ...
 */
/datum/runechat/proc/enter_subsystem(new_sched_destruction = 0)
	...
	if (new_sched_destruction)
		if (scheduled_destruction >= BUCKET_LIMIT)
			...
			if (bucket_pos > 0)
				var/datum/runechat/bucket_head = bucket_list[bucket_pos]
				if (bucket_head == src)
	...
	scheduled_destruction = max(NONUNIT_CEILING(scheduled_destruction, world.tick_lag), world.time + world.tick_lag)

	// Handle insertion into the secondary queue if the required time is outside our tracked amounts
	if (scheduled_destruction >= BUCKET_LIMIT)
		BINARY_INSERT(src, SSrunechat.second_queue, /datum/runechat, src, scheduled_destruction, COMPARE_KEY)
		return
...
/datum/runechat/proc/leave_subsystem()
	...
	var/datum/runechat/bucket_head
	if (bucket_pos > 0)
