...
/mob/examine_damage_state(mob/user)
	...
	if (!is_alive())
		to_chat(user, SPAN_DANGER("[pronouns.He] looks severely hurt and is not moving or responding to anything around [pronouns.him]."))
		return

	var/health_percentage = get_damage_percentage()
	switch (health_percentage)
		if (1)
			to_chat(user, SPAN_NOTICE("[pronouns.He] appears unhurt."))
		if (0.66 to 0.99)
			to_chat(user, SPAN_WARNING("[pronouns.He] looks slightly hurt."))
		if (0.33 to 0.65)
			to_chat(user, SPAN_WARNING("[pronouns.He] looks moderately hurt."))
		else
			to_chat(user, SPAN_DANGER("[pronouns.He] looks severely hurt."))
