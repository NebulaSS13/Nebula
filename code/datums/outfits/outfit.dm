...
/decl/hierarchy/outfit/proc/equip_base(mob/living/carbon/human/H, var/equip_adjustments)
	...
	if(uniform)
		H.equip_to_slot_or_del(new uniform(H), BP_CHEST)
		if(!H.get_equipped_item(BP_CHEST))
			H.equip_to_slot_or_del(new /obj/item/clothing/under/harness, BP_CHEST)
	if(holster && H.w_uniform)
		var/obj/item/clothing/accessory/equip_holster = new holster
		new_uniform.attackby(H, equip_holster)
		if(equip_holster.loc != new_uniform)
			qdel(equip_holster)
	if(suit)
		H.equip_to_slot_or_del(new suit(H), BP_BODY)
	if(back)
		H.equip_to_slot_or_del(new back(H),BP_SHOULDERS)
	if(belt)
		H.equip_to_slot_or_del(new belt(H),BP_GROIN)
	if(gloves)
		H.equip_to_slot_or_del(new gloves(H),slot_gloves_str)
	if(shoes)
		H.equip_to_slot_or_del(new shoes(H),slot_shoes_str)
	if(mask)
		H.equip_to_slot_or_del(new mask(H), BP_MOUTH)
	if(head)
		H.equip_to_slot_or_del(new head(H), BP_HEAD)
	if(l_ear)
		...
		H.equip_to_slot_or_del(new l_ear_path(H),BP_L_EAR)
	if(r_ear)
		...
		H.equip_to_slot_or_del(new r_ear_path(H),BP_R_EAR )
	if(glasses)
		H.equip_to_slot_or_del(new glasses(H),BP_EYES)
	if(id)
		H.equip_to_slot_or_del(new id(H),BP_NECK)
	if(l_pocket)
	...
	if((flags & OUTFIT_HAS_BACKPACK) && !(OUTFIT_ADJUSTMENT_SKIP_BACKPACK & equip_adjustments))
		...
		if(backpack)
			if(back)
				...
				H.equip_to_slot_or_del(backpack, BP_SHOULDERS)
