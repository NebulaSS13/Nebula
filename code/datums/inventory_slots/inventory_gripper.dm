...
/datum/inventory_slot/gripper/update_overlay(mob/living/user, obj/item/prop)
	. = ..()
	user.update_inv_hands()

/datum/inventory_slot/gripper/equipped(mob/living/user, obj/item/prop, var/silent = FALSE)
	. = ..()
	if(.)
...
/datum/inventory_slot/gripper/mouth/equipped(mob/living/user, obj/item/prop, var/silent = FALSE)
	. = ..()
	if(.)
		...
		if(istype(prop, /obj/item/chems/food))
			...
			qdel(prop)
			if(trash)
				equipped(user, new trash(user), silent = TRUE)
