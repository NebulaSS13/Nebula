...
/datum/inventory_slot/lower_body/update_mob_equipment_overlay(var/mob/living/user, var/obj/item/prop, var/redraw_mob = TRUE)
	...
	if(_holding && (!suit || !(suit.flags_inv & HIDEJUMPSUIT)))
		user.set_current_mob_overlay(HO_LOWER_BODY_LAYER, _holding.get_mob_overlay(user, slot_lower_body_str), redraw_mob)
	else
		user.set_current_mob_overlay(HO_LOWER_BODY_LAYER, null, redraw_mob)
