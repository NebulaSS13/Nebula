...
/datum/progress_bar/New(mob/user, goal, atom/target)
	...
	if(user)
		...
		animate(holder, pixel_z = world.icon_size + (height * (list_index - 1)), alpha = 255, time = 0.5 SECONDS, easing = SINE_EASING | EASE_OUT)
	else
...
/datum/progress_bar/proc/shift_down()
	--list_index
	var/shift_height = holder.pixel_z - height
	animate(holder, pixel_z = shift_height, time = 0.5 SECONDS, easing = SINE_EASING | EASE_OUT)
