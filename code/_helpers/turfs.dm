...
/proc/translate_turfs(list/translation, area/base_area = null, turf/base_turf, ignore_background, angle = 0)
	. = list()
	for(var/turf/source in translation)
		...
		if(target)
			if(base_area)
				...
				. += transport_turf_contents(source, target, ignore_background, angle = angle)
...
/proc/transport_turf_contents(turf/source, turf/target, ignore_background, angle = 0)
