...
/obj/item/organ/external/head/on_update_icon()
	...
	if(owner)
		...
		if(owner.lip_style && !BP_IS_PROSTHETIC(src) && (species && (species.appearance_flags & HAS_LIPS)))
			var/icon/lip_icon = new/icon(species ? species.lip_icon : 'icons/mob/human_races/species/human/lips.dmi', "lips_[owner.lip_style]_s")
			overlays |= lip_icon
...
/obj/item/organ/external/head/proc/get_hair_icon()
	...
	if(owner.f_style)
		...
		if(facial_hair_style)
			if(!facial_hair_style.species_allowed || (species.get_root_type(owner) in facial_hair_style.species_allowed))
				if(!facial_hair_style.subspecies_allowed || (species.type in facial_hair_style.subspecies_allowed))
	...
	if(owner.h_style)
		...
		if(hair_style)
			if(!hair_style.species_allowed || (species.get_root_type(owner) in hair_style.species_allowed))
				if(!hair_style.subspecies_allowed || (species.type in hair_style.subspecies_allowed))
