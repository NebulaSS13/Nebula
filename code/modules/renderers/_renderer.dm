...
/mob
	var/list/atom/movable/renderer/renderers


/// Creates the mob's renderers on /Login()
/mob/proc/create_renderers()
	...
	for (var/atom/movable/renderer/zrenderer as anything in global.zmimic_renderers)
		if (zrenderer.relay)
			my_client.screen += zrenderer.relay
		my_client.screen += zrenderer

/// Removes the mob's renderers on /Logout()
/mob/proc/remove_renderers()
	if(my_client)
		for(var/atom/movable/renderer/renderer as anything in renderers)
			my_client.screen -= renderer
			if (renderer.relay)
				my_client.screen -= renderer.relay
		for (var/atom/movable/renderer/renderer as anything in global.zmimic_renderers)
			my_client.screen -= renderer
	QDEL_NULL_LIST(renderers)
...
var/global/list/zmimic_renderers = list()
/hook/startup/proc/create_global_renderers() //Some (most) renderers probably do not need to be instantiated per mob. So may as well make them global and just add to screen
	...
	for(var/i = 0 to OPENTURF_MAX_DEPTH)
		global.zmimic_renderers += new /atom/movable/renderer/shared/zmimic(null, null, OPENTURF_MAX_PLANE - i)
	return TRUE
