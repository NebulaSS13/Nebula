		...
		for(var/obj/machinery/camera/C2 in CL)
			...
			if(C1 != C2)
				if(C1.c_tag == C2.c_tag)
					output += "<li><font color='red'>c_tag match for sec. cameras at \[[C1.x], [C1.y], [C1.z]\] ([C1_area.display_name]) and \[[C2.x], [C2.y], [C2.z]\] ([C2_area.display_name]) - c_tag is [C1.c_tag]</font></li>"
				if(C1.loc == C2.loc && C1.dir == C2.dir && C1.pixel_x == C2.pixel_x && C1.pixel_y == C2.pixel_y)
					output += "<li><font color='red'>FULLY overlapping sec. cameras at \[[C1.x], [C1.y], [C1.z]\] ([C1_area.display_name]) Channels: [C1.preset_channels] and [C2.preset_channels]</font></li>"
				if(C1.loc == C2.loc)
					output += "<li>overlapping sec. cameras at \[[C1.x], [C1.y], [C1.z]\] ([C1_area.display_name]) Channels: [C1.preset_channels] and [C2.preset_channels]</font></li>"
			...
			if(!(locate(/obj/structure/grille,T)))
				...
				for(var/obj/structure/window/W in T)
					if (W.dir == turn(C1.dir,180) || (W.dir in list(NORTHEAST,SOUTHEAST,SOUTHWEST,NORTHWEST)) )
						...
						break
				if(!window_check)
					output += "<li><font color='red'>Camera not connected to wall at \[[C1.x], [C1.y], [C1.z]\] ([C1_area.display_name]) Channels: [C1.preset_channels]</color></li>"
