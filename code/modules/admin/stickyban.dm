/datum/admins/proc/stickyban(action,data)
	...
	switch (action)
		...
		if ("revert")
			...
			log_admin("[key_name(usr)] has reverted [ckey]'s sticky ban to its state at round start.")
...
/datum/admins/proc/stickyban_get_html(ckey, ban)
	. = {"
		<a href='?_src_=holder;stickyban=remove&ckey=[ckey]'>\[-\]</a>
		<a href='?_src_=holder;stickyban=revert&ckey=[ckey]'>\[revert\]</a>
		<b>[ckey]</b>
		<br>
		Reason: [ban["message"]] <b><a href='?_src_=holder;stickyban=edit&ckey=[ckey]'>\[Edit\]</a></b><br>
	"}
	. += "Admin: [ban["admin"] ? ban["admin"] : "LEGACY"]<br>"
	. += "Caught keys:<br>\n<ol>"
	for (var/key in ban["keys"])
		if (ckey(key) == ckey)
			continue
...
/datum/admins/proc/stickyban_show()
	...
	var/list/bans = sortTim(world.GetConfig("ban"), /proc/cmp_text_asc)
	var/banhtml = ""
	for(var/key in bans)
		var/ckey = ckey(key)
		var/ban = stickyban2list(world.GetConfig("ban",key))
		banhtml += "<br><hr />\n"
		banhtml += stickyban_get_html(ckey,ban)
