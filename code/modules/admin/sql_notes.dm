/proc/add_note(target_ckey, notetext, timestamp, adminckey, logged = 1, server)
	if(!check_rights(R_INVESTIGATE, 1, usr)) 
		return
	...
	if(logged)
		...
		send2adminirc("[get_key(usr)] has added a note to [target_ckey]: \"[notetext]\"")
...
/proc/remove_note(note_id)
	...
	send2adminirc("[get_key(usr)] has removed a note made by [adminckey] from [ckey]: \"[notetext]\"")
	show_note(ckey)
...
/proc/edit_note(note_id)
	...
	if(query_find_note_edit.NextRow())
		...
		send2adminirc("[get_key(usr)] has edited [target_ckey]'s note made by [adminckey] - from \"[old_note]\" to - \"[new_note]\"")
		show_note(target_ckey)
...
/proc/show_note(target_ckey, index, linkless = 0)
	...
	for(var/letter in GLOB.full_alphabet)
