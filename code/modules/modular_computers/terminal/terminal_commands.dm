...
/datum/terminal_command/proc/parse(text, mob/user, datum/terminal/terminal)
	...
	if(network_card)
		...
		if(network)
			return proper_input_entered(text, user, terminal, network)
		return "Hardware error. No active exonet connection detected."
	return "Hardware error. No connected network interface detected."


//Should not return null unless you want parser to continue.
/datum/terminal_command/proc/proper_input_entered(text, mob/user, terminal, var/datum/exonet/network)
	return list()
...
/datum/terminal_command/man/proper_input_entered(text, mob/user, datum/terminal/terminal, var/datum/exonet/network)
	if(text == "man")
...
/datum/terminal_command/ifconfig/proper_input_entered(text, mob/user, datum/terminal/terminal, var/datum/exonet/network)
	var/obj/item/stock_parts/computer/network_card/network_card = terminal.computer.get_component(PART_NETWORK)
	...
	return "Real nid: [network_card.identification_id]."
...
/datum/terminal_command/hwinfo/proper_input_entered(text, mob/user, datum/terminal/terminal, var/datum/exonet/network)
	if(text == "hwinfo")
...
// Sysadmin
/datum/terminal_command/broadcasters
	name = "broadcasters"
	man_entry = list("Format: broadcasters", "Gives the number of active broadcasters found on the network.")
	pattern = "^broadcasters$"
	req_access = list(access_network)

/datum/terminal_command/broadcasters/proper_input_entered(text, mob/user, terminal, var/datum/exonet/network)
	return "Number of broadcasters found: [network.broadcasters.len]"
...
/datum/terminal_command/banned/proper_input_entered(text, mob/user, terminal, var/datum/exonet/network)
	. = list()
...
/datum/terminal_command/status/proper_input_entered(text, mob/user, terminal, var/datum/exonet/network)
	. = list()
	. += "EXONET status: [network.check_function() ? "ENABLED" : "DISABLED"]"
...
// /datum/terminal_command/locate/proper_input_entered(text, mob/user, datum/terminal/terminal, var/datum/exonet/network)
