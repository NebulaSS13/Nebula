...
/datum/nano_module/deck_management/Topic(href, href_list)
	...
	if(href_list["report"])
		...
		if(set_shuttle(user, shuttle_name, 0) && set_mission(mission_ID))
			...
			if(href_list["flight_plan"])
				...
				if(!is_valid_index(index, report_prototypes))
					return 1
				var/datum/computer_file/report/prototype = report_prototypes[index]
				prog_state = DECK_REPORT_EDIT
	...
	if(href_list["summon_crew"])
		...
		if(world.time - my_log.last_spam >= 1 MINUTE) //Slow down with that spam button
			global.announcer.autosay("The [selected_shuttle.name] is planning to depart on a mission promptly at [time]. The following crew members are to make their way to \the [place] immediately: [crew].", "Hangar Announcement System")
