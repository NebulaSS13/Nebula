...
/datum/nano_module/program/network_monitor/ui_interact(mob/user, ui_key = "main", var/datum/nanoui/ui = null, var/force_open = NANOUI_FOCUS, var/datum/topic_state/state = GLOB.default_state)
	...
	if(network)
		...
		if(length(network.mainframes_by_role[MF_ROLE_LOG_SERVER]))
			var/list/logs[0]
			for(var/datum/extension/network_device/mainframe/M in network.mainframes_by_role[MF_ROLE_LOG_SERVER])
				var/list/logdata[0]
