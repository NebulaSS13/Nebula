...
/datum/report_field/options/crew_record/rank/proc/record_ranks()
	...
	var/datum/mil_branch/branch = global.using_map.mil_branches.get_branch(record.get_branch())
	if(!branch)
		return
	. = list()
	. |= "Unset"
...
/datum/report_field/options/crew_record/branch/proc/record_branches()
	. = list()
	. |= "Unset"
	for(var/branch in global.using_map.mil_branches.branches)
		var/datum/mil_branch/branch_datum = global.using_map.mil_branches.branches[branch]
		. |= branch_datum.name
