...
// Attempts to create a log in exonet datum. Returns 1 on success, 0 on fail.
...
/datum/computer_file/program/proc/update_netspeed()
	...
	switch(exonet_status)
		if(1)
			net_speed = NETWORKSPEED_LOWSIGNAL
		if(2)
			net_speed = NETWORKSPEED_HIGHSIGNAL
		if(3)
			net_speed = NETWORKSPEED_ETHERNET
...
/datum/nano_module/program/proc/get_functional_component(var/component_type)
	var/obj/item/stock_parts/computer/component = os && os.get_component(component_type)
...
/datum/nano_module/program/proc/get_exonet_device()
	var/obj/item/stock_parts/computer/network_card/network_card = get_functional_component(/obj/item/stock_parts/computer/network_card)
	if(!network_card)
