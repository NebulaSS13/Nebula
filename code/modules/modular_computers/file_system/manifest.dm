...
/proc/html_crew_manifest(var/monochrome, var/OOC, records = global.all_crew_records)
	...
	for(var/datum/computer_file/report/crew_record/CR in records)
		...
		if(global.using_map.flags & MAP_HAS_RANK)
			var/datum/mil_branch/branch_obj = global.using_map.mil_branches.get_branch(CR.get_branch())
			var/datum/mil_rank/rank_obj = global.using_map.mil_branches.get_rank(CR.get_branch(), CR.get_rank())
