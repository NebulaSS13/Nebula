...
/obj/structure/reagent_dispensers/attackby(obj/item/W, mob/user)
	if(isWrench(W))
		...
		if(unwrenched)
			log_and_message_admins("opened a tank at [get_area(loc).display_name].")
	...
	if(Proj.get_structure_damage())
		if(istype(Proj.firer))
			...
			if(turf)
				...
				log_and_message_admins("[key_name_admin(Proj.firer)] shot a fuel tank in \the [area.display_name].")
			else
