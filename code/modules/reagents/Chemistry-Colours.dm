/datum/reagents/proc/get_color()
	...
	if(LAZYLEN(reagent_volumes) == 1) // It's pretty common and saves a lot of work
		...
		return R.icon_colour + num2hex(Floor(255 * R.opacity))

	var/list/colors = list(0, 0, 0, 0)
	var/tot_w = 0
	for(var/rtype in reagent_volumes)
		var/decl/material/R = decls_repository.get_decl(rtype)
		if(R.icon_colour_weight <= 0)
			continue
		var/hex = uppertext(R.icon_colour) + num2hex(Floor(255 * R.opacity))
		var/mod = REAGENT_VOLUME(src, rtype) * R.icon_colour_weight
