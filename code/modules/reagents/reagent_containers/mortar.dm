...
/obj/item/chems/glass/mortar/attackby(obj/item/using_item, mob/user)
	if((. = ..()))
		return
	...
	var/skill_factor = CLAMP01(1 + 0.3*(user.get_skill_value(SKILL_BOTANY) - SKILL_EXPERT)/(SKILL_EXPERT - SKILL_MIN))
	if(!crushing_item)
		...
		return TRUE
	else if(using_item.force > 0 && attacking_material && crushing_material)
		...
		if(REAGENTS_FREE_SPACE(reagents) < crushing_item.reagents.total_volume)
			to_chat(user, SPAN_WARNING("\The [src] is too full to grind \the [src], it'd spill everywhere!"))
			return TRUE
		if(crushing_item.reagents.total_volume) // if it has no reagents, skip all the fluff and destroy it instantly
			var/stamina_to_consume = max(crushing_item.reagents.total_volume * max(0.25, user.get_stamina_skill_mod()), 5)
			...
			if(!do_after(user, stamina_to_consume))
