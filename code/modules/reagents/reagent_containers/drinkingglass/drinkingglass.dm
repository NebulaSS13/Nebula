...
/obj/item/chems/food/drinks/glass2/proc/has_ice()
	if(LAZYLEN(reagents.reagent_volumes))
		...
		if(!((R.type == MAT_WATER) || ("ice" in R.glass_special))) // if it's not a cup of ice, and it's not already supposed to have ice in, see if the bartender's put ice in it
			if(reagents.has_reagent(MAT_WATER, reagents.total_volume / 10)) // 10% ice by volume
				return 1
