...
/datum/category_item/player_setup_item/controls/keybindings/OnTopic(href, list/href_list, mob/user)
	switch(href_list["preference"])
		...
		if("keybindings_set")
			...
			pref.key_bindings[full_key] = sort_list(pref.key_bindings[full_key], /proc/cmp_text_asc)
		...
		if("keybinding_reset")
			...
			for(var/key in kb.hotkey_keys)
				...
				pref.key_bindings[key] = sort_list(pref.key_bindings[key], /proc/cmp_text_asc)
			user.client.set_macros()
			return TOPIC_REFRESH
