...
/datum/preferences
	...
	var/list/bgstate_options = list("000", "FFF", MAT_STEEL, "white")
...
/datum/category_item/player_setup_item/physical/body/OnTopic(var/href,var/list/href_list, var/mob/user)
	...
	if(href_list["toggle_species_verbose"])
		...
		if(choice)
			var/decl/species/current_species = decls_repository.get_decl(choice)
		...
		for(var/species in get_playable_species())
			if(!check_rights(R_ADMIN, 0) && config.usealienwhitelist)
				var/decl/species/current_species = decls_repository.get_decl(species)
		...
		var/decl/species/choice = input("Select a species to play as.") as null|anything in species_to_pick
		if(!istype(choice))
			return
		...
		if(prev_species != pref.species)
			mob_species = decls_repository.get_decl(pref.species)
			...
			if(!has_flag(decls_repository.get_decl(pref.species), HAS_UNDERWEAR))
		...
		if(new_hair && has_flag(decls_repository.get_decl(pref.species), HAS_HAIR_COLOR) && CanUseTopic(user))
		...
		if(new_facial && has_flag(decls_repository.get_decl(pref.species), HAS_HAIR_COLOR) && CanUseTopic(user))
		...
		if(new_eyes && has_flag(decls_repository.get_decl(pref.species), HAS_EYE_COLOR) && CanUseTopic(user))
		...
		if(new_skin && has_flag(decls_repository.get_decl(pref.species), HAS_SKIN_COLOR) && CanUseTopic(user))
		...
		for(var/M in usable_markings)
			...
			if(is_type_in_list(S, disallowed_markings) || (S.species_allowed && !(mob_species.get_root_type() in S.species_allowed)) || (S.subspecies_allowed && !(mob_species.type in S.subspecies_allowed)))
				usable_markings -= M
		...
		switch(new_state)
			...
			if("Prosthesis")
				var/decl/species/temp_species = decls_repository.get_decl(pref.species || GLOB.using_map.default_species)
