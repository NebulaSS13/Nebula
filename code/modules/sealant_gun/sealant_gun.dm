/obj/item/gun/launcher/sealant
	...
	on_mob_icon =  'icons/obj/guns/sealant_gun.dmi'
	icon_state =    "world"
	can_autofire =  TRUE
	has_safety =    FALSE
	waterproof =    TRUE
...
/obj/item/gun/launcher/sealant/apply_overlays(mob/user_mob, bodytype, image/overlay, slot)
	. = ..()
	if(loaded_tank)
...
/obj/item/gun/launcher/sealant/attack_hand(mob/user)
	if(user.get_inactive_hand() == src && loaded_tank)
		unload_tank(user)
		return TRUE
	. = ..()
