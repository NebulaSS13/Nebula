...
/decl/codex_category/languages/Populate()
	...
	for(var/langname in language_types)
		var/decl/language/language = language_types[langname]
		if(language.hidden_from_codex)
			continue
		var/list/lang_info = list()
		var/decl/prefix/P = /decl/prefix/language
		lang_info += "Key to use it: '[initial(P.default_key)][language.language_key]'"
		if(language.language_flags & LANG_FLAG_NONVERBAL)
			lang_info += "It has a significant non-verbal component. Speech is garbled without line-of-sight."
		if(language.language_flags & LANG_FLAG_SIGNLANG)
			lang_info += "It is completely non-verbal, using gestures or signs to communicate."
		if(language.language_flags & LANG_FLAG_HIVEMIND)
			lang_info += "It's a 'hivemind' language, broadcast to all creatures who understand it."
		if(language.language_flags & LANG_FLAG_NO_STUTTER)
			lang_info += "It will not be affected by speech impediments."

		var/list/lang_lore = list(language.desc)
		lang_lore += "Shorthand: '[language.shorthand]'"
		if(!(language.language_flags & (LANG_FLAG_SIGNLANG|LANG_FLAG_NONVERBAL|LANG_FLAG_HIVEMIND)))
			var/lang_example = language.format_message(language.scramble(null, example_line), language.speech_verb)
			lang_lore += "It sounds like this:"
			lang_lore += ""
			lang_lore += "<b>CodexBot</b> [lang_example]"

		var/datum/codex_entry/entry = new(
			_display_name = "[language.name] (language)",
