...
/datum/codex_category/fusion_reaction/Initialize()
	...
	for(var/rtype in reactions)
		...
		var/decl/material/p_mat = SSmaterials.get_material_datum(reaction.p_react)
		var/decl/material/s_mat = SSmaterials.get_material_datum(reaction.s_react)
		...
		if(LAZYLEN(reaction.products))
			...
			for(var/P in reaction.products)
				if(ispath(P, /decl/material))
					var/decl/material/product_mat = SSmaterials.get_material_datum(P)
