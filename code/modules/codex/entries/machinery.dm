...
/datum/codex_entry/pacman
	...
	mechanics_text = "Lends itself to being portable thanks to the small size and ease of use. Some versions uses radioactive fuel and as such produces radiation, while others may produce dangerous byproducts as gasses. Ideally one should wear protective gear while interacting with an active generator. While active it also produces heat and will eventually overheat and explode. While the power output can be increased, doing this causes it to heat up faster. Must be secured using a wrench before use."
