...
/obj/machinery/power/rad_collector/Process()
	...
	if(P)
		if(P.air_contents.gas[MATERIAL_PHORON] == 0)
			investigate_log("<font color='red'>out of fuel</font>.","singulo")
			eject()
		else
			P.air_adjust_gas(MATERIAL_PHORON, -0.01*drainratio*min(last_rads,max_rads)/max_rads) //fuel cost increases linearly with incoming radiation
...
/obj/machinery/power/rad_collector/interface_interact(mob/user)
	...
	if(!src.locked)
		...
		investigate_log("turned [active?"<font color='green'>on</font>":"<font color='red'>off</font>"] by [user.key]. [P?"Fuel: [round(P.air_contents.gas[MATERIAL_PHORON]/0.29)]%":"<font color='red'>It is empty</font>"].","singulo")
	else
...
/obj/machinery/power/rad_collector/proc/receive_pulse(var/pulse_strength)
	if(P && active)
		...
		power_produced = min(100*P.air_contents.gas[MATERIAL_PHORON]*pulse_strength*pulse_coeff,max_power)
