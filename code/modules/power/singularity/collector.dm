...
/obj/machinery/rad_collector/Process()
	...
	if(loaded_tank)
		if(!loaded_tank.air_contents.get_gas(/decl/material/solid/phoron))
...
/obj/machinery/rad_collector/interface_interact(mob/user)
	...
	if(!src.locked)
		...
		investigate_log("turned [active?"<font color='green'>on</font>":"<font color='red'>off</font>"] by [user.key]. [loaded_tank?"Fuel: [round(loaded_tank.air_contents.get_gas(/decl/material/solid/phoron)/0.29)]%":"<font color='red'>It is empty</font>"].","singulo")
	else
...
/obj/machinery/rad_collector/proc/receive_pulse(var/pulse_strength)
	if(loaded_tank && active)
		...
		power_produced = min(100*loaded_tank.air_contents.get_gas(/decl/material/solid/phoron)*pulse_strength*pulse_coeff,max_power)
