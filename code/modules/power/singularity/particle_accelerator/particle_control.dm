...
/obj/machinery/particle_accelerator/control_box/proc/add_strength(var/s)
	if(assembled)
		...
		if(strength > strength_upper_limit)
			...
			log_and_message_admins("[src] increased to [strength] by [usr.ckey]([usr]) at ([x],[y],[z]).")
		strength_change()
...
/obj/machinery/particle_accelerator/control_box/proc/remove_strength(var/s)
	if(assembled)
		...
		if(strength < 0)
			...
			log_and_message_admins("[src] decreased to [strength] by [usr.ckey]([usr]) at ([x],[y],[z]).")
		strength_change()
...
/obj/machinery/particle_accelerator/control_box/Process()
	if(src.active)
		...
		if( length(connected_parts) < 6 )
			log_and_message_admins("[src] lost a connected part and powered down at ([x],[y],[z]).")
			src.toggle_power()
			return
		//emit some particles
...
/obj/machinery/particle_accelerator/control_box/toggle_power()
	...
	log_and_message_admins("[src] turned [active ?"ON":"OFF"] by [usr ? usr.key : "outside forces"] at ([x],[y],[z]).")
	if(src.active)
