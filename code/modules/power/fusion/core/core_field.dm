...
/obj/effect/fusion_em_field/proc/handle_tick()
	...
	if(uptake_gas && uptake_gas.total_moles)
		for(var/gasname in GET_GAS_LIST(uptake_gas))
			if(GET_GAS(uptake_gas, gasname)*10 > reactants[gasname])
				AddParticles(gasname, GET_GAS(uptake_gas, gasname)*10)
				uptake_gas.adjust_gas(gasname, -(GET_GAS(uptake_gas, gasname)), update=FALSE)
