/obj/item/brain_interface
	...
	var/robot_brain_noun = "Robot"
	var/holder_type = /obj/item/organ/internal/brain_holder/organic
...
/obj/item/brain_interface/proc/update_from_brain()
	if(holding_brain?.brainmob)
		holding_brain.brainmob.SetName(initial(holding_brain.name))
	update_icon()
...
/obj/item/brain_interface/Initialize()
	if(type == /obj/item/brain_interface)
		PRINT_STACK_TRACE("Abstract brain interface type ([type]) tried to Initialize(), use a subtype!")
...
/obj/item/brain_interface/proc/transfer_player(var/mob/M) // Same deal as the regular brain proc. Used for human-->robot people.
	if(holding_brain)
		holding_brain.transfer_identity(M, FALSE)
