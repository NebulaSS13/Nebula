...
/datum/nano_module/law_manager/Topic(href, href_list)
	...
	if(href_list["add_ion_law"])
		if(ion_law && is_jailbroken(usr))
			owner.add_ion_law(ion_law)
		return 1

	if(href_list["add_inherent_law"])
		if(inherent_law && is_jailbroken(usr))
			owner.add_inherent_law(inherent_law)
		return 1

	if(href_list["add_supplied_law"])
		if(supplied_law && supplied_law_position >= 1 && MIN_SUPPLIED_LAW_NUMBER <= MAX_SUPPLIED_LAW_NUMBER && is_jailbroken(usr))
	...
	if(href_list["edit_law"])
		if(is_jailbroken(usr))
			...
			if(AL)
				...
				if(new_law && new_law != AL.law && is_jailbroken(usr) && can_still_topic())
...
/datum/nano_module/law_manager/ui_interact(mob/user, ui_key = "main", var/datum/nanoui/ui = null, var/force_open = 1, var/datum/topic_state/state = global.default_topic_state)
	...
	data["isMalf"] = is_jailbroken(user)
	...
	if (!ui)
		ui = new(user, src, ui_key, "law_manager.tmpl", sanitize("[src] - [owner]"), 800, is_jailbroken(user) ? 600 : 400, state = state)
