...
/obj/item/integrated_circuit/smart/advanced_pathfinder/do_work()
	...
	var/list/P = get_path_to(a_loc, locate(get_pin_data(IC_INPUT, 1), get_pin_data(IC_INPUT, 2), a_loc.z), max_distance = 200, id = idc, exclude = get_turf(get_pin_data_as_type(IC_INPUT, 3, /atom)))
