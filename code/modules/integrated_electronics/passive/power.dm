...
/obj/item/integrated_circuit/passive/power/chemical_cell
	...
	spawn_flags = IC_SPAWN_DEFAULT|IC_SPAWN_RESEARCH
	var/volume = 60
	var/list/fuel = list(/decl/material/toxin/phoron = 50000, /decl/material/fuel = 15000, /decl/material/carbon = 10000, /decl/material/ethanol = 10000, /decl/material/nutriment = 8000)
	var/multi = 1
	var/lfwb =TRUE
...
/obj/item/integrated_circuit/passive/power/chemical_cell/make_energy()
	if(assembly)
		if(assembly.battery)
			var/bp = 5000
			if((assembly.battery.maxcharge-assembly.battery.charge) / CELLRATE > bp && reagents.remove_reagent(/decl/material/blood, 1)) //only blood is powerful enough to power the station(c)
