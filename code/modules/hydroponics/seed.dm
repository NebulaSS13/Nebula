...
/datum/seed/proc/do_photosynthesis(var/turf/current_turf, var/datum/gas_mixture/environment, var/light_supplied)
	...
	if(!(light_supplied) || !(get_trait(TRAIT_REQUIRES_WATER)))
		return
	if(environment.get_gas(MATERIAL_CO2) >= req_CO2_moles)
		environment.adjust_gas(MATERIAL_CO2, -req_CO2_moles, 1)
...
/datum/seed/proc/randomize()
	...
	if(prob(5))
		consume_gasses = list()
		var/gas = pick(MATERIAL_OXYGEN,MATERIAL_NITROGEN,MATERIAL_PHORON,MATERIAL_CO2)
