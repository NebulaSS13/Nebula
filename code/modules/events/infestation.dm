...
/datum/event/infestation/start()
	...
	spawn(0)
		...
		log_and_message_admins("Vermin infestation spawned ([vermstring] x[num]) in \the [location.display_name]", location = pick_area_turf(location))
...
/datum/event/infestation/announce()
	command_announcement.Announce("Bioscans indicate that [vermstring] have been breeding in \the [location.display_name]. Further infestation is likely if left unchecked.", "[location_name()] Biologic Sensor Network", zlevels = affecting_z)
...
/datum/event/infestation/proc/set_location_get_infestation_turfs()
	...
	if(!vermin_turfs.len)
		log_debug("Vermin infestation failed to find viable turfs in \the [location.display_name].")
		return
	return vermin_turfs
