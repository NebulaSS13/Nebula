...
/obj/screen/legend/Initialize(mapload, map_color, text)
	...
	maptext = STYLE_SMALLFONTS_OUTLINE("<a href='?src=\ref[src]'>[text]</a>", 6, map_color, COLOR_BLACK)
	alpha = 254
	...
	if(length(global.using_map.overmap_ids))
		...
		if(istype(O))
			...
			for(var/level = 1; level <= z_count; level++)
				...
				maptext_overlay.maptext = STYLE_SMALLFONTS_OUTLINE("<center>LEVEL [level-1]</center>", 9, COLOR_WHITE, COLOR_BLACK)
