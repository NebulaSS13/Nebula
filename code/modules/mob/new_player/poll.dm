...
/mob/new_player/proc/poll_player(var/pollid = -1)
	...
	if(dbcon.IsConnected())
		...
		switch(polltype)
			...
			//Polls with a text input
			if("NUMVAL")
				var/DBQuery/voted_query = dbcon.NewQuery("SELECT `text`, `rating` FROM `erro_poll_option` INNER JOIN `erro_poll_vote` USING (`pollid`) WHERE `ckey` = '[usr.ckey]' AND `erro_poll_option`.`id` = `optionid`")
				voted_query.Execute()
