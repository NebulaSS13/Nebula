...
/mob/living/carbon/human/proc/get_valid_hairstyle_types(var/check_gender = 1)
	. = list()
	var/list/hair_styles = species.get_hair_style_types()
	for(var/hair_style in hair_styles)
		var/decl/sprite_accessory/S = hair_styles[hair_style]
		if(check_gender)
