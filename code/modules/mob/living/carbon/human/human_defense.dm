...
/mob/living/carbon/human/get_armors_by_zone(obj/item/organ/external/def_zone, damage_type, damage_flags)
	...
	if(!def_zone)
		return
	var/list/protective_gear = list(get_equipped_item(BP_HEAD), get_equipped_item(BP_MOUTH), get_equipped_item(BP_BODY), get_equipped_item(BP_CHEST), gloves, shoes)
...
/mob/living/carbon/human/proc/get_siemens_coefficient_organ(var/obj/item/organ/external/def_zone)
	...
	for(var/bp in inventory_slots)
		...
		if(inv_slot?.holding?.body_parts_covered & def_zone.body_part) // Is that body part being targeted covered?
			siemens_coefficient *= inv_slot.holding.siemens_coefficient
...
/mob/living/carbon/human/proc/check_head_coverage()
	for(var/bp in inventory_slots)
		var/datum/inventory_slot/inv_slot = inventory_slots[bp]
		if(inv_slot?.holding?.body_parts_covered & SLOT_HEAD)
			return 1
	return 0
...
/mob/living/carbon/human/check_mouth_coverage()
	for(var/bp in inventory_slots)
		var/datum/inventory_slot/inv_slot = inventory_slots[bp]
		if(inv_slot?.holding && (inv_slot.holding.body_parts_covered & SLOT_FACE) && (inv_slot.holding.item_flags & ITEM_FLAG_FLEXIBLEMATERIAL))
			return inv_slot.holding

/mob/living/carbon/human/proc/check_shields(var/damage = 0, var/atom/damage_source = null, var/mob/attacker = null, var/def_zone = null, var/attack_text = "the attack")
	var/list/checking_slots = get_held_items()
	LAZYDISTINCTADD(checking_slots, get_equipped_item(BP_BODY))
...
/mob/living/carbon/human/proc/handle_suit_punctures(var/damtype, var/damage, var/def_zone)
	...
	var/obj/item/clothing/suit/space/suit = get_equipped_item(BP_BODY)
	if(istype(suit))
