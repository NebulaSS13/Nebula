				...
				for(var/A in tile)
					if(istype(A, /obj/effect))
						...
						if(cleaned_human.lying)
							var/obj/item/head = cleaned_human.get_equipped_item(BP_HEAD)
							if(head)
								head.clean_blood()
								cleaned_human.update_inv_head(0)
							var/obj/item/uniform = cleaned_human.get_equipped_item(BP_CHEST)
							var/obj/item/suit = cleaned_human.get_equipped_item(BP_BODY)
							if(suit)
								suit.clean_blood()
