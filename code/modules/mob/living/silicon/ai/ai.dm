...
/mob/living/silicon/ai/reset_view(atom/A)
	...
	if(istype(A,/obj/machinery/camera))
		if(camera_light_on)
			A.set_light_new(AI_CAMERA_LUMINOSITY)
		else
			A.set_light(0)
		...
		if(src.camera)
			...
			if(camera && src.camera != camera)
				...
				if(!camera.light_disabled)
					src.camera = camera
					src.camera.set_light_new(AI_CAMERA_LUMINOSITY)
				else
					src.camera = null
...
/mob/living/silicon/ai/on_update_icon()
	...
	if(stat == DEAD)
		...
		set_light_new(3, 1, selected_sprite.dead_light)
	else if(!has_power())
		icon_state = selected_sprite.nopower_icon
		set_light_new(1, 1, selected_sprite.nopower_light)
	else
		icon_state = selected_sprite.alive_icon
		set_light_new(1, 1, selected_sprite.alive_light)
