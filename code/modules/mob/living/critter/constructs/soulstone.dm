/obj/item/soulstone
	...
	var/is_evil = 1
	var/mob/living/critter/shade = null
	var/smashing = 0
	var/soulstatus = null

/obj/item/soulstone/Initialize(var/mapload)
	shade = new /mob/living/critter/shade(src)
	. = ..(mapload)
...
/obj/item/soulstone/attack(var/mob/living/critter/M, var/mob/user)
	if(M == shade)
...
/obj/structure/constructshell/attackby(var/obj/item/I, var/mob/user)
	if(istype(I, /obj/item/soulstone))
		...
		switch(construct)
			if("Artificer")
				ctype = /mob/living/critter/construct/builder
			if("Wraith")
				ctype = /mob/living/critter/construct/wraith
			if("Juggernaut")
				ctype = /mob/living/critter/construct/armoured
		var/mob/living/critter/construct/C = new ctype(get_turf(src))
		C.key = S.shade.key
		//C.cancel_camera()
		if(S.is_evil)
