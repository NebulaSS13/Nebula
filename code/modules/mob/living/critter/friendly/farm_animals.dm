...
/mob/living/critter/hostile/retaliate/goat/Initialize()
	. = ..()
	udder = new(50, src)

/mob/living/critter/hostile/retaliate/goat/Destroy()
	QDEL_NULL(udder)
	. = ..()

/mob/living/critter/hostile/retaliate/goat/Life()
	. = ..()
	if(.)
...
/mob/living/critter/hostile/retaliate/goat/Retaliate()
	..()
	if(stat == CONSCIOUS && prob(50))
		visible_message("<span class='warning'>\The [src] gets an evil-looking gleam in their eye.</span>")

/mob/living/critter/hostile/retaliate/goat/attackby(var/obj/item/O, var/mob/user)
...
/mob/living/critter/cow/Initialize()
	. = ..()
	udder = new(50, src)

/mob/living/critter/cow/attackby(var/obj/item/O, var/mob/user)
	var/obj/item/chems/glass/G = O
	if(stat == CONSCIOUS && istype(G) && ATOM_IS_OPEN_CONTAINER(G))
		user.visible_message("<span class='notice'>[user] milks [src] using \the [O].</span>")
		var/transfered = udder.trans_type_to(G, /decl/material/liquid/drink/milk, rand(5,10))
...
/mob/living/critter/cow/attack_hand(mob/M)
...
/mob/living/critter/chick/Initialize()
	. = ..()
	pixel_x = rand(-6, 6)
	pixel_y = rand(0, 10)

/mob/living/critter/chick/Life()
	. = ..()
	if(!.)
		return FALSE
	amount_grown += rand(1,2)
	if(amount_grown >= 100)
		new /mob/living/critter/chicken(src.loc)
		qdel(src)
...
/mob/living/critter/chicken/Initialize()
	. = ..()
	if(!body_color)
...
/mob/living/critter/chicken/death(gibbed, deathmessage, show_dead_message)
	..(gibbed, deathmessage, show_dead_message)
	chicken_count -= 1

/mob/living/critter/chicken/attackby(var/obj/item/O, var/mob/user)
...
/mob/living/critter/chicken/Life()
	. = ..()
	if(!.)
		return FALSE
...
/obj/item/chems/food/snacks/egg/Process()
	if(isturf(loc))
		...
		if(amount_grown >= 100)
			...
			new /mob/living/critter/chick(get_turf(src))
			STOP_PROCESSING(SSobj, src)
			qdel(src)
	else
		return PROCESS_KILL
