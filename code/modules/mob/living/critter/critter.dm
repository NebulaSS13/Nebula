/mob/living/critter
	name = "animal"
	icon = 'icons/mob/critter/animal.dmi'
	health = 20
	maxHealth = 20
	...
	var/can_escape = FALSE // 'smart' critters such as human enemies, or things small, big, sharp or strong enough to power out of a net
	...
	var/speed = 0 // Movement delay modifier - higher is slower, negative is faster
	...
	//for critters with abilities, mostly megafauna
	var/ability_cooldown
	var/time_last_used_ability

	//for critters that reflect damage when attacked in melee
	var/return_damage_min
	var/return_damage_max

	var/performing_delayed_life_action = FALSE

/mob/living/critter/Initialize()
	. = ..()
...
/mob/living/critter/Destroy()
	if(istype(natural_weapon))
		QDEL_NULL(natural_weapon)
	. = ..()
	
/mob/living/critter/Life()
	. = ..()
	if(!.)
		return FALSE
...
/mob/living/critter/proc/delayed_life_action()
	set waitfor = FALSE
...
/mob/living/critter/proc/do_delayed_life_action()
...
/mob/living/critter/proc/handle_atmos(var/atmos_suitable = 1)
	//Atmos
...
/mob/living/critter/proc/escape(mob/living/M, obj/O)
	O.unbuckle_mob(M)
	visible_message("<span class='danger'>\The [M] escapes from \the [O]!</span>")

/mob/living/critter/proc/handle_supernatural()
	if(purge)
		purge -= 1

/mob/living/critter/gib()
	..(icon_gib,1)

/mob/living/critter/proc/visible_emote(var/act_desc)
	custom_emote(1, act_desc)

/mob/living/critter/proc/audible_emote(var/act_desc)
	custom_emote(2, act_desc)

/mob/living/critter/bullet_act(var/obj/item/projectile/Proj)
...
/mob/living/critter/attack_hand(mob/user)
	..()
...
/mob/living/critter/attackby(var/obj/item/O, var/mob/user)
...
/mob/living/critter/hit_with_weapon(obj/item/O, mob/living/user, var/effective_force, var/hit_zone)
...
/mob/living/critter/movement_delay()
...
/mob/living/critter/Stat()
	. = ..()

	if(statpanel("Status") && show_stat_health)
		stat(null, "Health: [round((health / maxHealth) * 100)]%")

/mob/living/critter/death(gibbed, deathmessage = "dies!", show_dead_message)
...
/mob/living/critter/explosion_act(severity)
	..()
	var/damage
	switch(severity)
		if(1)
...
/mob/living/critter/adjustBruteLoss(damage)
	..()
	updatehealth()

/mob/living/critter/adjustFireLoss(damage)
	..()
	updatehealth()

/mob/living/critter/adjustToxLoss(damage)
	..()
	updatehealth()

/mob/living/critter/adjustOxyLoss(damage)
	..()
	updatehealth()

/mob/living/critter/proc/SA_attackable(target_mob)
...
/mob/living/critter/get_speech_ending(verb, var/ending)
	return verb

/mob/living/critter/put_in_hands(var/obj/item/W) // No hands.
	W.forceMove(get_turf(src))
	return 1

// Harvest an animal's delicious byproducts
/mob/living/critter/proc/harvest(var/mob/user, var/skill_level)
...
/mob/living/critter/proc/subtract_meat(var/mob/user)
	meat_amount--
	if(meat_amount <= 0)
		to_chat(user, SPAN_NOTICE("\The [src] carcass is ruined beyond use."))

/mob/living/critter/bullet_impact_visuals(var/obj/item/projectile/P, var/def_zone)
	..()
...
/mob/living/critter/handle_fire()
	return

/mob/living/critter/update_fire()
	return
/mob/living/critter/IgniteMob()
	return
/mob/living/critter/ExtinguishMob()
	return

/mob/living/critter/is_burnable()
	return heat_damage_per_tick

/mob/living/critter/proc/adjustBleedTicks(var/amount)
	if(!can_bleed)
		return
...
/mob/living/critter/proc/handle_bleeding()
	bleed_ticks--
	adjustBruteLoss(1)
...
/mob/living/critter/get_digestion_product()
	return /decl/material/liquid/nutriment

/mob/living/critter/eyecheck()
...
/mob/living/critter/proc/reflect_unarmed_damage(var/mob/living/carbon/human/attacker, var/damage_type, var/description)
...
/mob/living/critter/proc/get_natural_weapon()
	if(ispath(natural_weapon))
		natural_weapon = new natural_weapon(src)
	return natural_weapon

/mob/living/critter/getCloneLoss()
	. = max(0, gene_damage)

/mob/living/critter/adjustCloneLoss(var/amount)
	setCloneLoss(gene_damage + amount)

/mob/living/critter/setCloneLoss(amount)
	if(gene_damage >= 0)
		gene_damage = Clamp(amount, 0, maxHealth)
		if(gene_damage >= maxHealth)
			death()

/mob/living/critter/get_admin_job_string()
	return "Animal"

/mob/living/critter/get_telecomms_race_info()
