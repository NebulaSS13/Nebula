...
/mob/living/critter/hostile/proc/can_act()
	if(stat || stop_automation || incapacitated())
		return FALSE
	return TRUE

/mob/living/critter/hostile/proc/kick_stance()
	if(target_mob)
		stance = HOSTILE_STANCE_ATTACK
	else
		stance = HOSTILE_STANCE_IDLE

/mob/living/critter/hostile/proc/FindTarget()
	if(!can_act())
		return null
...
/mob/living/critter/hostile/proc/ValidTarget(var/atom/A)
	if(A == src)
		return FALSE
...
/mob/living/critter/hostile/proc/Found(var/atom/A)
	return

/mob/living/critter/hostile/proc/MoveToTarget()
	if(!can_act())
		return
	if(confused)
...
/mob/living/critter/hostile/proc/AttackTarget()
	stop_automated_movement = 1
	if(!target_mob || SA_attackable(target_mob))
		LoseTarget()
		return 0
...
/mob/living/critter/hostile/proc/AttackingTarget()
...
/mob/living/critter/hostile/proc/LoseTarget()
	stance = HOSTILE_STANCE_IDLE
	target_mob = null
	walk(src, 0)

/mob/living/critter/hostile/proc/LostTarget()
	stance = HOSTILE_STANCE_IDLE
	walk(src, 0)

/mob/living/critter/hostile/proc/ListTargets(var/dist = 7)
	var/list/L = hearers(src, dist)
	return L

/mob/living/critter/hostile/proc/get_accuracy()
	return Clamp(sa_accuracy - melee_accuracy_mods(), 0, 100)

/mob/living/critter/hostile/death(gibbed, deathmessage, show_dead_message)
	..(gibbed, deathmessage, show_dead_message)
	walk(src, 0)

/mob/living/critter/hostile/Life()
	. = ..()
	if(!.)
		walk(src, 0)

/mob/living/critter/hostile/do_delayed_life_action()
	..()
	if(!can_act())
		walk(src, 0)
		kick_stance()
...
/mob/living/critter/hostile/attackby(var/obj/item/O, var/mob/user)
...
/mob/living/critter/hostile/attack_hand(mob/M)
	. = ..()
	if(M.a_intent == I_HURT && !incapacitated(INCAPACITATION_KNOCKOUT))
		target_mob = M
		MoveToTarget()

/mob/living/critter/hostile/bullet_act(var/obj/item/projectile/Proj)
	var/oldhealth = health
	. = ..()
...
/mob/living/critter/hostile/proc/OpenFire(target_mob)
...
/mob/living/critter/hostile/proc/Shoot(var/target, var/start, var/user, var/bullet = 0)
	if(target == start)
		return
...
/mob/living/critter/hostile/proc/DestroySurroundings() //courtesy of Lohikar
	if(!can_act())
		return
...
/mob/living/critter/hostile/proc/pry_door(var/mob/user, var/delay, var/obj/machinery/door/pesky_door)
...
/mob/living/critter/hostile/proc/can_perform_ability()
	if(!can_act() || time_last_used_ability > world.time)
		return FALSE
	return TRUE

/mob/living/critter/hostile/proc/cooldown_ability(var/time)
	if(!time)
