/mob/living/critter/hostile/commanded
	name = "commanded"
...
/mob/living/critter/hostile/commanded/hear_say(var/message, var/verb = "says", var/decl/language/language = null, var/alt_name = "", var/italics = 0, var/mob/speaker = null, var/sound/speech_sound, var/sound_vol)
	if((weakref(speaker) in friends) || speaker == master)
		command_buffer.Add(speaker)
		command_buffer.Add(lowertext(html_decode(message)))
	return 0

/mob/living/critter/hostile/commanded/hear_radio(var/message, var/verb="says", var/decl/language/language=null, var/part_a, var/part_b, var/part_c, var/mob/speaker = null, var/hard_to_hear = 0)
	if((weakref(speaker) in friends) || speaker == master)
		command_buffer.Add(speaker)
		command_buffer.Add(lowertext(html_decode(message)))
	return 0

/mob/living/critter/hostile/commanded/Life()
	. = ..()
	if(!.)
		return FALSE
...
/mob/living/critter/hostile/commanded/FindTarget(var/new_stance = HOSTILE_STANCE_ATTACK)
...
/mob/living/critter/hostile/commanded/proc/follow_target()
...
/mob/living/critter/hostile/commanded/proc/commanded_stop() //basically a proc that runs whenever we are asked to stay put. Probably going to remain unused.
	return

/mob/living/critter/hostile/commanded/proc/listen(var/mob/speaker, var/text)
...
/mob/living/critter/hostile/commanded/proc/get_targets_by_name(var/text, var/filter_friendlies = 0)
...
/mob/living/critter/hostile/commanded/proc/attack_command(var/mob/speaker,var/text)
...
/mob/living/critter/hostile/commanded/proc/stay_command(var/mob/speaker,var/text)
	target_mob = null
	stance = COMMANDED_STOP
	stop_automated_movement = 1
	walk_to(src,0)
...
/mob/living/critter/hostile/commanded/proc/stop_command(var/mob/speaker,var/text)
...
/mob/living/critter/hostile/commanded/proc/follow_command(var/mob/speaker,var/text)
...
/mob/living/critter/hostile/commanded/proc/misc_command(var/mob/speaker,var/text)
	return 0


/mob/living/critter/hostile/commanded/hit_with_weapon(obj/item/O, mob/living/user, var/effective_force, var/hit_zone)
...
/mob/living/critter/hostile/commanded/attack_hand(mob/M)
	..()
