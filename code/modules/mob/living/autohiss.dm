...
/mob/living/carbon/human/handle_autohiss(message, decl/language/L)
	if(!client || get_preference_value(/datum/client_preference/autohiss) == global.PREF_OFF) // no need to process if there's no client or they have autohiss off
		return message
...
/decl/species/proc/get_autohiss_map(var/mode)
	...
	if(mode == global.PREF_FULL && autohiss_extra_map)
		. |= autohiss_extra_map
