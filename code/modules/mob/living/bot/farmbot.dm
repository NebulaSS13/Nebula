...
/mob/living/bot/farmbot/UnarmedAttack(var/atom/A, var/proximity)
	...
	if(istype(A, /obj/machinery/portable_atmospherics/hydroponics))
		...
		switch(t)
			...
			if(FARMBOT_NUTRIMENT)
				...
				if(do_after(src, 30, A))
					...
					T.reagents.add_reagent_by_id(/decl/material/gas/ammonia, 10)
		busy = 0
		action = ""
		update_icon()
		T.update_icon()
		...
		while(do_after(src, 10) && tank.reagents.total_volume < tank.reagents.maximum_volume)
			tank.reagents.add_reagent_by_id(/decl/material/liquid/water, 100)
			if(prob(5))
				playsound(loc, 'sound/effects/slosh.ogg', 25, 1)
		busy = 0
		action = ""
...
/mob/living/bot/farmbot/confirmTarget(var/atom/targ)
	...
	else if(refills_water && tray.waterlevel < 40 && !tray.reagents.has_reagent_of_id(/decl/material/liquid/water))
