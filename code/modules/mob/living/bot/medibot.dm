...
/mob/living/bot/medbot/UnarmedAttack(var/mob/living/carbon/human/H, var/proximity)
	...
	if(declare_treatment)
		...
		broadcast_medical_hud_message("[src] is treating <b>[H]</b> in <b>[location.display_name]</b>", src)
	busy = 1
	update_icon()
	if(do_mob(src, H, 30))
		if(t == 1)
...
/mob/living/bot/medbot/proc/handle_panic()
	...
	switch(tipped_status)
		...
		if(MEDBOT_PANIC_END)
			broadcast_medical_hud_message("PSYCH ALERT: Crewmember [tipper_name] recorded displaying antisocial tendencies torturing bots in [get_area(src).display_name]. Please schedule psych evaluation.", src)
