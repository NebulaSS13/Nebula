...
/mob/living/bot/proc/startPatrol()
	...
	if(T)
		patrol_path = get_path_to(get_turf(loc), T, max_distance = max_patrol_dist, id = botcard, exclude = obstacle)
		if(!patrol_path)
			patrol_path = list()
		obstacle = null
	return
...
/mob/living/bot/proc/calcTargetPath()
	target_path = get_path_to(get_turf(loc), get_turf(target), max_distance = max_target_dist, id = botcard, exclude = obstacle)
	if(!target_path)
		if(target && target.loc)
			ignore_list |= target
		resetTarget()
