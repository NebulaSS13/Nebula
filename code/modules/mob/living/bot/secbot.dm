...
/mob/living/bot/secbot/proc/begin_arrest(mob/target, var/threat)
	...
	if(declare_arrests)
		broadcast_security_hud_message("[src] is arresting a level [threat] suspect <b>[suspect_name]</b> in <b>[get_area(src).proper_name]</b>.", src)
...
/mob/living/bot/secbot/proc/react_to_attack(mob/attacker)
	if(!target)
		...
		broadcast_security_hud_message("[src] was attacked by a hostile <b>[target_name(attacker)]</b> in <b>[get_area(src).proper_name]</b>.", src)
	target = attacker
	awaiting_surrender = INFINITY
