...
/obj/machinery/computer/design_console/ui_interact(mob/user, ui_key = "main", var/datum/nanoui/ui = null, var/force_open = 1)
	...
	if(viewing_database)
		...
		for(var/obj/machinery/destructive_analyzer/az in lan?.network_entities[/obj/machinery/destructive_analyzer])
...
/obj/machinery/computer/design_console/proc/process_analyzer(var/obj/machinery/destructive_analyzer/analyzer)
	...
	var/datum/extension/local_network_member/fabnet = get_extension(src, /datum/extension/local_network_member)
	var/datum/local_network/lan = fabnet?.get_local_network()
	if(!(analyzer in lan?.network_entities[/obj/machinery/destructive_analyzer]))
		return
...
/obj/machinery/computer/design_console/proc/update_fabricators()
	...
	if(!lan)
		return
	var/list/techs = get_network_tech_levels()
	for(var/machine_type in lan.network_entities)
		if(ispath(machine_type, /obj/machinery/fabricator))
			for(var/obj/machinery/fabricator/fab in lan?.network_entities[machine_type])
