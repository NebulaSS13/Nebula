...
/spell/proc/adjust_var(mob/living/target = usr, type, amount) //handles the adjustment of the var when the spell is used. has some hardcoded types
	switch(type)
		if("bruteloss")
			target.take_damage(BRUTE, amount)
		if("fireloss")
			target.take_damage(BURN, amount)
		if("toxloss")
			target.take_damage(TOX, amount)
		if("oxyloss")
			target.take_damage(OXY, amount)
		if("brainloss")
			target.take_damage(BRAIN, amount)
		if("stunned")
