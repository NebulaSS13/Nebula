...
/decl/species/proc/handle_npc(var/mob/living/carbon/human/H)
	return
...
/decl/species/proc/get_hair_styles()
	...
	if(!L)
		...
		for(var/hairstyle in GLOB.hair_styles_list)
			...
			if(S.species_allowed && !(get_root_type() in S.species_allowed))
				continue
			if(S.subspecies_allowed && !(type in S.subspecies_allowed))
				continue
	...
	if(!facial_hair_style_by_gender)
		...
		for(var/facialhairstyle in GLOB.facial_hair_styles_list)
			...
			if(gender == FEMALE && S.gender == MALE)
				continue
			if(S.species_allowed && !(get_root_name() in S.species_allowed))
				continue
			if(S.subspecies_allowed && !(type in S.subspecies_allowed))
				continue
