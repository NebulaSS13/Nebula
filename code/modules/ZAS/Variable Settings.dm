...
/vs_control/proc/SetDefault(var/mob/user)
	...
	switch(def)
		if("Toxins - Standard")
			...
			plc.TOXINSGUARD_ONLY = 0
			plc.GENETIC_CORRUPTION = 0  // Chance of genetic corruption as well as toxic damage, X in 1000.
			plc.SKIN_BURNS = 0          // Toxins have an effect similar to mustard gas on the un-suited.
			plc.EYE_BURNS = 1           // Toxins burn the eyes of anyone not wearing eye protection.
			plc.TOXINS_HALLUCINATION = 0
			plc.CONTAMINATION_LOSS = 0.02

		if("Toxins - Low Hazard")
			plc.CLOTH_CONTAMINATION = 0 // If this is on, toxins do damage by getting into cloth.
			plc.TOXINSGUARD_ONLY = 0
			plc.GENETIC_CORRUPTION = 0  // Chance of genetic corruption as well as toxic damage, X in 1000
			plc.SKIN_BURNS = 0          // Toxins have an effect similar to mustard gas on the un-suited.
			plc.EYE_BURNS = 1           // Toxins burn the eyes of anyone not wearing eye protection.
		...
		if("Toxins - High Hazard")
			plc.CLOTH_CONTAMINATION = 1 // If this is on, toxins do damage by getting into cloth.
			plc.TOXINSGUARD_ONLY = 0
			plc.GENETIC_CORRUPTION = 0  // Chance of genetic corruption as well as toxic damage, X in 1000.
			plc.SKIN_BURNS = 1          // Toxins have an effect similar to mustard gas on the un-suited.
			plc.EYE_BURNS = 1           // Toxins burn the eyes of anyone not wearing eye protection.
			plc.TOXINS_HALLUCINATION = 1
			plc.CONTAMINATION_LOSS = 0.05

		if("Toxins - Oh Shit!")
			plc.CLOTH_CONTAMINATION = 1 // If this is on, toxins do damage by getting into cloth.
			plc.TOXINSGUARD_ONLY = 1
		...
		if("ZAS/Toxins - Initial")
			...
			plc.TOXINS_DMG 					= initial(plc.TOXINS_DMG)
			plc.CLOTH_CONTAMINATION 		= initial(plc.CLOTH_CONTAMINATION)
			plc.TOXINSGUARD_ONLY 			= initial(plc.TOXINSGUARD_ONLY)
			...
			plc.TOXINS_HALLUCINATION 		= initial(plc.TOXINS_HALLUCINATION)
			plc.N2O_HALLUCINATION 			= initial(plc.N2O_HALLUCINATION)


	to_world("<span class='notice'><b>[key_name(user)] changed the global contaminants/ZAS settings to \"[def]\"</b></span>")
