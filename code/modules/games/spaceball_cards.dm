...
/obj/item/pack/spaceball/Initialize()
	...
	for(i=0;i<5;i++)
		...
		if(prob(1))
			...
			var/decl/language/language = GET_DECL(/decl/language/human/common)
			var/team = pick("Brickburn Galaxy Trekers","Mars Rovers", "Qerrbalak Saints", "Moghes Rockets", "Ahdomai Lightening")
			P.name = "[language.get_random_language_name(pick(MALE,FEMALE))], [global.using_map.game_year - rand(0,50)] [team]"
