...
/datum/controller/subsystem/persistence/proc/SaveWorld()
	...
	try
		...
		for(var/datum/persistence/load_cache/z_level/z_level in z_transform)
		...
		for(var/datum/persistence/load_cache/z_level/z_level in z_transform)
			z_inserts += "([z_insert_index],[z_level.new_index],[z_level.dynamic],'[z_level.default_turf]')"
			z_insert_index++
		var/DBQuery/query = dbcon.NewQuery("INSERT INTO `z_level` (`id`,`z`,`dynamic`,`default_turf`) VALUES[jointext(z_inserts, ",")]")
		query.Execute()
		if(query.ErrorMsg())
...
/datum/controller/subsystem/persistence/proc/LoadWorld()
	try
		...
		for(var/datum/persistence/load_cache/z_level/z_level in serializer.resolver.z_levels)
			if(z_level.dynamic)
				z_level.new_index = INCREMENT_WORLD_Z_SIZE
