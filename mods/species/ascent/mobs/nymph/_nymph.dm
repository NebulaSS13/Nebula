...
/mob/living/simple_animal/alien/ascent_nymph/get_jump_distance()
	return 3

/mob/living/simple_animal/alien/ascent_nymph/Login()
	. = ..()
	if(client)
		if(holding_item)
...
/mob/living/simple_animal/alien/ascent_nymph/Initialize(var/mapload)
	update_icon()
	. = ..(mapload)
	set_extension(src, /datum/extension/base_icon_state, icon_state)

/mob/living/simple_animal/alien/ascent_nymph/show_examined_worn_held_items(mob/user, distance, infix, suffix, hideflags, decl/pronouns/pronouns)
	. = ..()
	if(holding_item)
