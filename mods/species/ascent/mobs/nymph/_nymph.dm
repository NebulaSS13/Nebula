...
/mob/living/alien/ascent_nymph/get_jump_distance()
	return 3

/mob/living/alien/ascent_nymph/Login()
	. = ..()
	if(client)
		if(holding_item)
...
/mob/living/alien/ascent_nymph/Initialize(var/mapload)
	update_icon()
	. = ..(mapload)
	set_extension(src, /datum/extension/base_icon_state, icon_state)

/mob/living/alien/ascent_nymph/show_examined_worn_held_items(mob/user, distance, infix, suffix, hideflags, decl/pronouns/pronouns)
	. = ..()
	if(holding_item)
		to_chat(user, SPAN_NOTICE("It is holding \icon[holding_item] \a [holding_item]."))

/mob/living/alien/ascent_nymph/has_dexterity()
	return FALSE

/mob/living/alien/ascent_nymph/death(gibbed)
	if(holding_item)
		unEquip(holding_item)

	return ..(gibbed,death_msg)

/mob/living/alien/ascent_nymph/on_update_icon()
	..()
	icon_state = ICON_STATE_WORLD
