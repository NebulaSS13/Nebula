...
/obj/item/gun/launcher/vox/get_examine_hints(mob/user, distance, infix, suffix)
	. = ..()
	LAZYADD(., SPAN_SUBTLE("Feeding meat to \the [src] will increase its firing force by [((fed_force_bonus - 1) * 100)]% for [/datum/extension/voxform::FED_TIME / (1 MINUTE)] minute\s."))
