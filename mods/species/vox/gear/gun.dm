...
/datum/extension/voxform/proc/check_held_user(var/mob/living/carbon/human/user, var/atom/movable/thing)
	if(!istype(user))
		return FALSE
	if(user.species.get_bodytype(user) != BODYTYPE_VOX && user.unEquip(thing))
...
/spell/targeted/shapeshift/true_form
	...
	possible_transformations = list(/mob/living/critter/hostile/retaliate/parrot/space/lesser)
