...
/spell/targeted/choose_targets(mob/user = usr)
	...
	if(max_targets == 0) //unlimited
		...
		if((range == 0 || range == -1) && !user_is_immune)
			targets += user
		else
			...
			for(var/mob/living/M in starting_targets)
				if(user_is_immune && M == user)
					continue
				if(faction_immune && user.faction == M.faction)
					continue
				if(faction_only && user.faction != M.faction)
					continue
			...
			if(possible_targets.len)
				if(can_select_target) //if we are allowed to choose. see setup.dm for details
	...
	if(user_is_immune && (user in targets))
		targets -= user
