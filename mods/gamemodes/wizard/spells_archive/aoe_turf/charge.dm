/decl/ability/wizard/charge
	...
	ability_icon_state = "wiz_charge"
	cast_sound = 'sound/magic/charge.ogg'

/decl/ability/wizard/charge/cast(var/list/targets, mob/user)
	for(var/turf/T in targets)
		depth_cast(T)

/decl/ability/wizard/charge/proc/depth_cast(var/list/targets)
	for(var/atom/A in targets)
		if(A.contents.len)
			depth_cast(A.contents)
		cast_charge(A)

/decl/ability/wizard/charge/proc/mob_charge(var/mob/living/M)
	if(!M.mind)
		return
	if(M.mind.learned_spells.len != 0)
		for(var/spell/S in M.mind.learned_spells)
			if(!istype(S, /decl/ability/wizard/charge))
...
/decl/ability/wizard/charge/proc/cast_charge(var/atom/target)
...
/decl/ability/wizard/charge/blood
	name = "Blood Infusion"
