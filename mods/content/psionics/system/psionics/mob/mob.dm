...
/mob/living/human/check_shields(var/damage = 0, var/atom/damage_source = null, var/mob/attacker = null, var/def_zone = null, var/attack_text = "the attack")
	...
	if(istype(P))
		...
		if(istype(starting) && !P.disrupts_psionics() && psi && prob(psi.get_armour(SSmaterials.get_armor_key(P.atom_damage_type, P.damage_flags())) * 0.5) && psi.spend_power(round(damage/10)))
			visible_message(SPAN_DANGER("\The [src] deflects [isatom(attack_text) ? "\the [attack_text]" : attack_text]!"))
			P.redirect(starting.x + rand(-2,2), starting.y + rand(-2,2), get_turf(src), src)
