...
/obj/item/integrated_circuit/verb/rename_component()
	...
	var/input = sanitize_name(input(M, "What do you want to name this?", "Rename", name) as null|text, allow_numbers = TRUE, force_first_letter_uppercase = FALSE)
	if(check_interactivity(M))
		if(!input)
			input = name
...
/obj/item/integrated_circuit/Topic(href, href_list, state = global.physical_topic_state)
	...
	if(href_list["pin"] && assembly)
		...
		if(pin)
			...
			if(istype(held_item, /obj/item/wirer) || istype(held_item, /obj/item/debugger))
		...
		if(istype(held_item, /obj/item/debugger))
			var/obj/item/debugger/D = held_item
			if(D.accepting_refs)
