...
/datum/integrated_io/proc/handle_wire(datum/integrated_io/linked_pin, obj/item/tool, action, mob/living/user)
	if(istype(tool, /obj/item/wirer))
		var/obj/item/wirer/wirer = tool
		if(linked_pin)
	...
	else if(istype(tool, /obj/item/debugger))
		var/obj/item/debugger/debugger = tool
