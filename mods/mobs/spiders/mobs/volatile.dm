...
/mob/living/simple_animal/hostile/giant_spider/volatile/proc/do_death_explosion()
	...
	animate(src, color = "#000000", time = 1, loop = delay/2, flags = ANIMATION_PARALLEL, easing = JUMP_EASING)
	animate(color = "#ff0000", time = 1, easing = JUMP_EASING)
	sleep(delay)
	color = initial(color)
	animate(src) // end animation if it's still ongoing
