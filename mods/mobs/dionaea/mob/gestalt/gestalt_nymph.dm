...
/obj/structure/diona_gestalt/proc/roll_up_atom(var/mob/living/simple_animal/alien/diona/chirp, var/silent)
	...
	if(istype(chirp, /mob/living/simple_animal/alien/diona))
		nymphs[chirp] = TRUE
		queue_icon_update()
	chirp.forceMove(src)
...
/obj/structure/diona_gestalt/proc/check_nymphs()
	if(LAZYLEN(nymphs.len) >= 2)
		for(var/mob/living/simple_animal/alien/diona/chirp in nymphs)
			if(chirp.client)
				return
	visible_message(SPAN_DANGER("\The [src] splits apart!"))
	qdel(src)
